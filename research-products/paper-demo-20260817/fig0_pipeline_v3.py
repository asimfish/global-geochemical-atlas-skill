# Figure 1 v3: methods overview. v2 + paper-figure style absorption:
# numbered reading-order badges, main path heavier than auxiliary paths.
import matplotlib
matplotlib.use('Agg')
...
def badge(num, x, y, color):
    ax.text(x, y, str(num), ha='center', va='center', fontsize=5.6, weight='bold',
...
for i in range(4):
    arrow((xs[i] + W + 0.25, Y + H/2), (xs[i+1] - 0.25, Y + H/2), lw=1.7)
for i in range(5):
    badge(i + 1, xs[i] + 1.1, Y + H - 0.4, BLUE)

# ---------- drop connector (carries cohorts) ----------
arrow((89.2, Y - 0.2), (89.2, 34.3), lw=1.8)
...
badge(7, 3.3, SY + SH - 0.4, RED)
arrow((SX - 0.3, SY + SH/2), (38.6, SY + SH/2), lw=1.8)
