...
| Fabricated completion or grade inflation | Missing agents remain `awaiting_agents`; failed review creates an immutable feedback-bound revision (maximum three). Passing review auto-publishes a hash-verified package at grade `camera_ready`; an exhausted budget can only publish at `draft_with_disclosed_findings` with every open finding copied verbatim into `publication_manifest.json`, so the grade cannot be inflated without breaking the manifest hash. Static pages only export a request. | Journal submission and any external sign-off remain outside GGA; the manifest records what the package actually cleared. |
