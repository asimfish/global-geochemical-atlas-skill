...
| AUTO-03 | Paper and figure agents cannot invent results, and cannot deliver an unpublishable layout. | All numerical manuscript claims must resolve through the atlas claim ledger or a hash-bound pilot result; unsupported claims block drafting/review. Figure specs cite the same claim IDs and data hashes. The manuscript is written on the controller-installed paper kit (`gga-paper-v1`: fixed style, skeleton, generated claim ledger, seeded bibliography) and cites numbers only through `\claimref` marks; the typesetting gate rejects sources that leave the kit, fixed figure heights, figures behind the bibliography, uncited or unlisted bib keys and PDFs not produced by a TeX engine. Figures are drawn with the figure kit (`gga-figure-kit-v1`) and the render-fidelity gate rejects PDF/PNG exports whose aspect differs from the SVG and spatial figures without the offline basemap layer. | typesetting/render-fidelity negative tests, kit-installation tests, real-data chain runs with pdfTeX + headless Chrome |
