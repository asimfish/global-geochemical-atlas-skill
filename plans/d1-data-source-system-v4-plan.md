...
更新时间：2026-08-06 21:49（Asia/Shanghai）

状态：执行中；V4-M0、M1、M2、M3 已完成，正在执行 M4 水体增强

承接版本：`plans/d1-data-source-system-v3-plan.md`

当前基线：分支 `d1/source-system-v2`，提交 `a8c7262`
...
2. **采样坐标**：联合演示 796 条均保留来源报告坐标，但只有 700 条具有可进入地图的 canonical EPSG:4326 坐标；GEOROC 的 datum 和 AfSIS 的 CRS 未证实，96 条坐标失败关闭。全量 profile 同样区分 549,510 个来源坐标样品与 523,745 个 canonical 坐标样品，不用数值范围合法替代 CRS 证据。
3. **地质背景**：语义混用已经清理。旧 `geologic_unit` 现为 0/796，原来的 192 条地区、图幅、航次或项目范围已迁入 `geographic_context_raw`、`map_sheet`、`cruise_track` 和 `survey_area`。`geologic_unit_raw/matched_geologic_unit` 同样为 0/796，说明当前没有足够证据支持“已采集地质单元”，后续必须补来源地质字段或版本化空间匹配。
4. **分析方法**：联合演示中 652/796（81.9%）非空且带 scope，但不同来源的粒度不一致，有的是逐记录方法，有的是批次、文件或数据集说明，有的完全缺失。演示主动选择了方法明确的 GEMStat 和 AfSIS 记录，因此仍会高估全量完整率。
5. **样品类型**：十四来源 M2 回填已完成。联合演示 796/796 有 `sample_type_raw + sample_type + mapping_status`；348 条土壤有层位，256 条沉积物有环境类型，144 条水体有水体类型和分相。映射只来自来源 evidence 或注册常量，不从元素、单位或方法推断。
...
8. **自动采集**：下载、缓存、快照、hash、适配器和来源定位链路已经存在；十四来源现已发布最近成功获取/解析、文件 hash、schema fingerprint、row-count、缓存/离线重放和版本漂移状态。在线实时探测仍与可复现离线审计分开运行。
...
### 2.2 304 条历史基线与 796 条当前基线的口径对齐

用户提供的 304 条检查对应 2026-08-05 的五来源联合 fixture：GEOROC、USGS、MarChem、GEOTRACES 和 GEMStat。它准确证明当时四介质流水线已打通；当前仓库已扩展为十四来源、796 条 fixture，因此 304 和 796 不能混成同一时点的完整率。

| 需求 | 304 条历史基线 | 796 条当前基线 | V4 采用的验收口径 |
|---|---|---|---|
| 岩石/土壤/沉积物/水体 | 四介质均有落地来源 | 四介质、十四个可执行数据集 | 同时报介质、样品类型、独立血缘和空间覆盖，不只判断“有/无” |
| 元素含量 | 304/304 有可处理元素记录；来源存在删失值时保留限定符和检出限 | 796/796 有元素、值/限定符、单位和来源定位 | 全量来源分别统计有效值、删失、缺失、单位可换算和来源定位比例 |
| 采样坐标 | 304/304 当时按查询坐标口径统计 | 796 条有 reported 坐标，700 条有 canonical 坐标；GEOROC/AfSIS 失败关闭 | 同时验收原始坐标、原 CRS、转换记录、canonical 坐标和不确定度；演示不替代全量 |
| 地质背景 | 岩石 100%、水体 50%、土壤/沉积物 0% 非空 | 来源/匹配地质单元 0/796；原 192 条地区/图幅/航线已拆到地理上下文 | 地理上下文、来源地质字段和空间匹配地质字段分开统计，语义错误不得算完整 |
| 分析方法 | 沉积物 100%、水体 50%，岩石/土壤 0%；矩阵标记 `not_yet_audited` | 652/796 有方法且全部带 scope；其余 144/796 有明确缺失原因 | 按 observation/sample/batch/file/dataset/publication scope 统计；全量而非优选 demo 作分母 |
...
下表统计的是当前 796 条 hash 固定联合演示，不是全量来源总体。

| 介质 | 演示记录 | 元素+值 | 坐标 | 来源/匹配地质单元 | 方法或缺失原因 | 样品类型 |
|---|---:|---:|---:|---:|---:|---:|
| 岩石 | 48 | 100% | 0% canonical | 0% | 100% | 100% |
| 土壤 | 348 | 100% | 86.2% canonical | 0% | 100% | 100% |
| 沉积物 | 256 | 100% | 100% | 0% | 100% | 100% |
| 水体 | 144 | 100% | 100% | 0% | 100% | 100% |
| 合计 | 796 | 100% | 87.9% canonical | 0% | 100% | 100% |

这里的“方法或缺失原因”不等于方法完整率：652 条有方法和 scope，144 条只有明确缺失原因。坐标列的 87.9% 只统计 canonical EPSG:4326，96 条 reported coordinates 仍保留在数据库。真正地质单元 0% 是清理错误语义后的诚实结果，不是数据被误删；192 条原值仍保留在地理上下文字段。
...
| 风险 | 证据 | 影响 | 严重度 | V4 处理 |
|---|---|---|---|---|
| 样品类型在交换层丢失 | 历史 27 列长表没有 `sample_type` 等字段 | 无法可靠按样品类型查询、制图和分组 | Resolved in M2 | 796/796 已回填并纳入生成器和回归测试 |
| 地理上下文冒充地质单元 | 历史 `LOCATION`、图幅、航线写入 `geologic_unit` | 按地质单元统计可能产生错误结论 | Resolved in M2 | 旧字段清零，192 条迁入四类地理上下文字段；真正地质单元仍为空 |
| 方法完整率被 demo 高估 | GEMStat demo 选方法明确记录，全量方法完整率实测 8.78% | 置信度和可比较记录数量被高估 | Resolved in M0 | 已发布 full/demo 两套 profile；覆盖立方体只把 43,285/492,999 条 GEMStat 测定计为最低可比 |
...
| 历史/当前 fixture 数字混用 | 304 与 796 属于不同版本 | 进展报告和覆盖率不可比较 | Medium | 所有报告携带 fixture/version/as_of 和分母 |
...
| 里程碑 | 预计 | 主要交付 | 完成标准 |
|---|---:|---|---|
| V4-M0：全量审计（完成） | 1 天 | 十四来源 full/demo completeness、文献、使用条件和自动化健康报告 | 14/14 verified cache 已统一逐字段解析；742,060 条全量目标测定与 796 条 demo 分开报告 |
...
| V4-M3：覆盖立方体（完成） | 1 天 | 六类数量指标、空间格网、介质平衡报告 | 5,773 个 source×medium×element×sample type×method×region×basis 单元，四介质均同时报告六类指标 |
...
| 2026-08-06 21:49 | V4-M0 全量审计与 M3 覆盖立方体 | 从 14 个 verified full cache 全量解析 742,060 条目标测定和 554,429 个逐来源去重样品；549,510 个样品有来源报告坐标，523,745 个具有 canonical EPSG:4326 坐标，94,023 条达到最低跨来源可比条件。生成 112 份分来源字段/方法/样品/空间/地质/引用/使用条件/自动化健康文件、5,773 行覆盖立方体和四介质平衡报告。GEOROC datum、AfSIS CRS 与 GSJ 全量转换链未证实时均不进入 canonical 覆盖。水体虽有 537,174 条测定但仅 3 条独立血缘；MarChem 外层 ZIP 漂移但数据与方法成员 hash 一致 | 执行 M4：补方法连接、真正地质背景与新的独立水体/沉积物血缘 |
