...
更新时间：2026-08-07 16:46（Asia/Shanghai）

状态：执行中；V4-M0 至 M5 已完成，正在执行 M6 土壤/岩石扩展
...
8. **自动采集**：下载、缓存、快照、适配器和来源定位链路已经存在；十八来源发布最近成功获取/解析、来源版本、文件名、字节数、字段清单、row-count、缓存/离线重放和版本漂移状态。V4 从本次调整起不再计算或验证任何文件哈希。
...
下表统计的是当前 792 条版本固定联合演示，不是全量来源总体。
...
| V4-M5：沉积物增强（完成） | 2–4 天 | NGSA、GSJ marine、PANGAEA Arabian Sea | 七条沉积物独立血缘显式分型；river/stream/floodplain/marine 与 TOS/BOS 不混合 |
