...
更新时间：2026-08-06 18:55（Asia/Shanghai）

状态：执行中；V4-M0 分口径 profile 框架与 M1 schema v2 基础链路已落地，M2 十四来源回填待执行

承接版本：`plans/d1-data-source-system-v3-plan.md`

当前基线：分支 `d1/source-system-v2`，提交 `766b31f`
...
5. **样品类型**：V4 schema v2 已增加 `sample_type`、`water_fraction`、`sediment_environment` 等字段，并打通归档、交换、标准输出、SQLite、GeoJSON 和地图筛选；但十四个既有来源尚未完成 M2 回填，当前联合演示这些新字段仍是 0/736。因此现在只能表述为“结构能力已具备”，不能表述为“生产字段已完整”。
