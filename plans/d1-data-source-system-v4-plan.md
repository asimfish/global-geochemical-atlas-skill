...
更新时间：2026-08-07 18:45（Asia/Shanghai）

状态：执行中；V4-M0 至 M5 已完成，M6 三个优先适配器已完成，仍缺第二条独立岩石血缘

承接版本：`plans/d1-data-source-system-v3-plan.md`

当前基线：分支 `codex/d1-v4-source-expansion`；M6 集成与联合验收变更在工作区

## 一、结论先行

当前系统已经证明采集架构能够覆盖四种介质和主要处理步骤，但并不是每类信息都同样完整。“架构能力存在”和“字段已充分填充”必须分开验收。

1. **元素含量**：当前二十一个可执行数据集都能输出元素、原始数值、单位和来源定位；全量共 4,086,778 条目标测定，联合演示 1,084/1,084 具备这些字段。但演示只抽取可用记录，不能代表全量数据的缺失率。
2. **采样坐标**：联合演示 1,084/1,084 有可用于地图的坐标；全量 771,322 个来源内去重样品中 766,402 个有有效坐标。缺失坐标、未声明 CRS 和坐标范围仍分别保留，不由演示切片掩盖。
...
4. **分析方法**：联合演示中 836/1,084 有方法及 scope，其余 248 条有明确缺失原因。全量口径差异很大：GEMStat 只有 279,225/3,739,180（7.47%）方法明确；GEOTRACES 有 22,254/39,327（56.59%）能唯一关联到 BODC 方法记录；GEOROC precompiled 成员不提供 winning method，保持缺失而不猜测。
5. **样品类型**：二十一来源均已贯通 V4 语义。联合演示 1,084/1,084 有 `sample_type_raw + sample_type + mapping_status`；380 条土壤按层位/用途区分，408 条沉积物有环境类型，200 条水体有水体类型和分相。映射只来自来源 evidence 或注册常量，不从元素、单位或方法推断。
...
8. **自动采集**：下载、缓存、快照、适配器和来源定位链路已经存在；二十一来源发布最近成功获取/解析、来源版本、文件名、字节数、字段清单、row-count、缓存/离线重放和版本漂移状态。V4 不计算或验证任何文件哈希。

### 1.1 V4 无哈希原则（2026-08-07 决策）
...
| 介质 | 可执行数据集数 | 数据集 |
|---|---:|---|
| 岩石 | 2 | GEOROC Archaean Cratons、GEOROC Antarctica Intraplate Volcanics（同属 GEOROC compilation lineage） |
| 土壤 | 8 | USGS CONUS、PANGAEA North Africa、FOREGS topsoil/subsoil/humus、AfSIS、TPDC China Mountain Soil、GEMAS Europe |
| 沉积物 | 7 | MarChem、GSJ river/marine、FOREGS stream/floodplain、NGSA Hg、PANGAEA Arabian Sea |
| 水体 | 4 | GEOTRACES、GEMStat、FOREGS stream water、USGS/WQP Sacramento River dissolved As |

岩石已有两个可执行数据集，但二者共享 GEOROC compilation lineage，不能算成两条独立上游证据；土壤来源数量最多；水体仍存在地区和分相不均。不能用单一“记录总数”判断哪种介质更全面。

### 2.2 304 条与 736 条历史基线的口径对齐

用户提供的 304 条检查对应 2026-08-05 的五来源联合 fixture；736 条对应 M2 完成时的十四来源版本；M4 为十五来源、792 条，M5 为十八来源、944 条。当前 M6 基线是二十一来源、1,084 条 fixture。这些数字都是各自时点的真实快照，不能混成同一时点的完整率。
...
下表统计的是当前 1,084 条版本固定联合演示，不是全量来源总体。
