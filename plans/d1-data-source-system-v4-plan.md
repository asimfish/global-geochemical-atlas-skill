...
状态：V4 核心目标已完成；继续滚动补源和维护

承接版本：`plans/d1-data-source-system-v3-plan.md`

当前基线：26 个可执行来源、4,096,615 条目标元素观测；独立血缘 rock 2、soil 7、sediment 8、water 5

## 一、结论先行

当前系统已经证明采集架构能够覆盖四种介质和主要处理步骤，但并不是每类信息都同样完整。“架构能力存在”和“字段已充分填充”必须分开验收。

1. **元素含量**：当前 26 个可执行数据集都能输出元素、原始数值、单位和来源定位；全量 profile 共 4,096,615 条目标测定。演示只抽取可用记录，不能代表全量数据的缺失率。
2. **采样坐标**：全量 773,445 个来源内去重样品中 768,525 个有有效坐标。缺失坐标、未声明 CRS 和坐标范围仍分别保留，不由演示切片掩盖。
3. **地质背景**：语义混用已经清理。旧 `geologic_unit` 在联合演示中保持为空，地区、图幅、航次或项目范围进入地理上下文字段。TPDC `Rock Group` 只作为 `lithology_raw`，GEOROC 岩性、年龄和构造背景保留为原始地质字段；没有版本化空间匹配证据时不生成规范地质单元。
4. **分析方法**：26 个来源均发布方法完整率或明确缺失原因。全量口径差异很大：GEMStat 只有 279,225/3,739,180（7.47%）方法明确；GEOTRACES 有 22,254/39,327（56.59%）能唯一关联到 BODC 方法记录；GEOROC precompiled 成员不提供 winning method，保持缺失而不猜测。
5. **样品类型**：26 个来源均已贯通 V4 语义并生成统一 full profile；映射只来自来源 evidence 或注册常量，不从元素、单位或方法推断。
...
8. **自动采集**：下载、缓存、快照、适配器和来源定位链路已经存在；26 个来源发布最近成功获取/解析、来源版本、文件名、字节数、字段清单、row-count、缓存/离线重放和版本漂移状态。V4 不计算或验证任何文件哈希。

### 1.1 日常运行与全量模式

- 默认按请求路由相关来源，优先使用小 fixture、已有来源分区和增量缓存；正常使用、来源准入和普通测试都不要求下载或持有 400 万行本地数据库。
- 全量下载、26 来源 full profile 和全量 SQLite 只用于显式发布、覆盖审计、性能验收或用户明确要求的离线总库构建。
- 全量能力必须可重放，但全量产物不是仓库运行时前置条件；无变更来源应复用中间分区，只处理新增或变化来源。

### 1.2 V4 无哈希原则（2026-08-07 决策）

- 原始文件、中间产物、fixture、报告和交付包都不再新计算 MD5、SHA-256 或其他内容哈希；
- 哈希不得作为来源准入、置信度、生产路由、缓存命中、漂移检测、回归测试或交付验收门槛；
- 历史文件中已存在的哈希字段可为兼容保留，但新流程不读取、不比对、不更新；
- 文件身份改用 `source_id + DOI/PID + dataset_version + file_id/file_name + publisher release date + accessed_at`；
- 工程漂移改用响应状态、文件字节数、成员清单、字段集、类型、行数、目标元素数和关键分类计数检测；
- 科学可信性仍由出版者、版本、方法、单位、介质、QC、文献、30 条复核和适用边界支撑，不用文件哈希替代。
...
| 介质 | 可执行数据集数 | 数据集 |
|---|---:|---|
| 岩石 | 3 | GEOROC Archaean、GEOROC Antarctica、USGS Utah volcanic whole-rock；独立血缘 2 |
| 土壤 | 9 | USGS CONUS、PANGAEA North Africa、FOREGS topsoil/subsoil/humus、AfSIS、TPDC、GEMAS、Brazil SGB；独立血缘 7 |
| 沉积物 | 9 | MarChem、GSJ river/marine、FOREGS stream/floodplain、NGSA、PANGAEA Arabian Sea、CDoGS lake、Brazil SGB；独立血缘 8 |
| 水体 | 5 | GEOTRACES、GEMStat、FOREGS stream water、USGS/WQP Sacramento River、CDoGS lake water；独立血缘 5 |

GEOROC 两个数据集共享 compilation lineage，USGS Utah 提供第二条独立岩石血缘。四介质都达到 V4 数量目标，但地区、元素、方法、时间和采样密度仍不均匀，不能用单一“记录总数”判断哪种介质更全面。

### 2.2 304 条与 736 条历史基线的口径对齐

用户提供的 304 条检查对应 2026-08-05 的五来源联合 fixture；736 条对应 M2 完成时的十四来源版本；M4 为十五来源、792 条，M5 为十八来源、944 条，M6 为二十一来源、1,084 条。当前 26 来源工程样板为 1,271 条。这些数字都是各自时点的真实快照，不能混成同一时点的完整率。
...
| 自动采集 | 脚本化流水线和 snapshot manifest | 十四个 adapter、离线 fixture 与联合重建 | 增加最近成功获取、版本/字段/行数漂移、缓存可读性和失败降级监控；不再使用哈希 |
...
下表保留 M6 的 1,084 条版本固定联合演示作为历史回归基线，不是当前 26 来源全量总体；当前覆盖结论以 4,096,615 条 full profile 和 1,271 条工程样板的机器报告为准。

| 介质 | 演示记录 | 元素+值 | 坐标 | 来源/匹配地质单元 | 方法或缺失原因 | 样品类型 |
|---|---:|---:|---:|---:|---:|---:|
| 岩石 | 96 | 100% | 100% | 0% | 100% | 100% |
| 土壤 | 380 | 100% | 100% | 0% | 100% | 100% |
| 沉积物 | 408 | 100% | 100% | 0% | 100% | 100% |
| 水体 | 200 | 100% | 100% | 0% | 100% | 100% |
| 合计 | 1,084 | 100% | 100% | 0% | 100% | 100% |

这里的“方法或缺失原因”不等于方法完整率：836 条有方法和 scope，248 条只有明确缺失原因。真正地质单元 0% 是清理错误语义后的诚实结果，不是数据被误删；地理上下文仍保留在专门字段中。
...
- 固定来源版本、文件名、字节数、成员清单、schema fingerprint 和行数基线；
...
1. **已完成**：接入 NGSA Australia Hg，固定产品 DOI、TOS/BOS 样品类型、单位、方法边界和坐标；0.01 ng Hg 是质量 LOD，不误作浓度检出限；
2. **已完成**：将 GSJ marine sediment 作为独立产品接入，不与现有 river sediment 混为一个样品类型；保留 4,905 个样品、37 个航次/区域和 Hg 负值/缺失边界；
3. **已完成首个样板**：接入 PANGAEA Arabian Sea DOI `10.1594/PANGAEA.950139`，27 行、26 个事件、162 条六元素测定；方法只保留出版者给出的 bulk-sediment 泛化描述；
...
- AfSIS Phase I：**已完成首轮工程闭环**。使用官方 Dataverse V2.0（2025-10-13 发布），固定三份 original 文件、文件 ID、版本和 DOI；对账 2,002 个唯一样品、18 个国家标签、51 个站点、上下层、六个目标元素、方法与全局 DL/QL。下一步是具名签署 30 条复核和将全量质量统计接入 V4 profile；
...
| 自动化健康 | 获取、解析、版本、schema、行数与离线重放可验证 | 漂移或下载失败时降级到最后可读快照并报告陈旧度；不做哈希校验 |
...
| V4-M5：沉积物增强（完成） | 2–4 天 | NGSA、GSJ marine、PANGAEA Arabian Sea | 七个可执行沉积物数据集归并为六条上游血缘（FOREGS stream/floodplain 共享项目血缘）；river/stream/floodplain/marine 与 TOS/BOS 不混合 |
| V4-M6：土壤/岩石扩展 | 进行中 | GEMAS、GEOROC Antarctica、TPDC adapter 已完成；继续补一条非 GEOROC 岩石血缘 | 三个优先来源遵守 V4 schema；独立岩石血缘仍未完成 |
...
12. adapter 必须检测版本、文件字节数、成员清单、schema、行数和关键分类计数漂移，并能从最后可读快照离线重放；不计算或检查哈希；
...
4. 为全部可执行来源补 method scope 和缺失原因；
...
|---|---|---|---|
| 2026-08-07 | V4 26 来源核心验收 | 新增 USGS Utah 独立岩石、Brazil SGB 土壤/河流沉积物和 CDoGS 湖水/湖泊沉积物等来源；26/26 具备 adapter、全量 profile、结构化 Codex 自动审计和离线 fixture。全量能力为 4,096,615 条测定、773,445 个来源内去重样品、768,525 个有效坐标样品和 434,661 条最低可比测定；独立血缘达到 rock 2、soil 7、sediment 8、water 5。覆盖仍为 `partial` | 日常按请求增量获取；全量下载/数据库仅在显式发布或验收模式重建，继续滚动补区域和方法空白 |
| 2026-08-07 18:45 | V4-M6 三个优先适配器与 21 来源联合验收 | 新增 GEOROC Antarctica 424 行/1,183 条岩石测定、TPDC 1,314 行/6,570 条土壤测定、GEMAS 4,131 个物理样品/8,262 个方法分离记录/53,703 条土壤测定。三者均完成 registry/catalog、adapter、V4 语义、全量 profile、无哈希 snapshot/reconciliation、30 条结构化自动审计和离线 demo。21 来源全量为 4,086,778 条测定、771,322 个样品、766,402 个坐标样品、426,493 条最低可比测定和 8,268 行覆盖立方体；上游独立血缘按介质为岩石 1、土壤 6、沉积物 6、水体 4；1,084 条联合 fixture 按构造背景等字段拆成 160 个比较组，22 个工程异常候选；191 项组件检查通过 | M6 仍需增加一条与 GEOROC 不同上游血缘的岩石来源；随后进入 M7 查询、地图、文档与交付自查 |
| 2026-08-07 17:57 | V4 无哈希迁移与 M5 沉积物增强完成 | 删除下载、适配、缓存、快照、标准化、证据包、SQLite、审计、profile 和测试中的哈希计算/校验路径；历史字段仅保留兼容。新增 NGSA Hg 2,396 条、GSJ marine 34,334 条、PANGAEA Arabian Sea 162 条测定，三个来源均完成 adapter、全量 profile、30 条结构化自动审计和离线 demo。18 来源全量重算为 4,025,322 条测定、765,564 个样品、366,220 条最低可比测定和 6,930 行覆盖立方体；944 条联合 fixture、126 个比较组、191 项组件检查和 22 项端到端检查通过 | 执行 M6：优先把 GEMAS、GEOROC Antarctica 与 TPDC 做成正式 V4 adapter；随后增加第二条独立岩石血缘并进入 M7 联合验收 |
| 2026-08-07 16:46 | V4 取消哈希计算与验收 | 计划明确停止对原始文件、中间产物、fixture、报告和交付包新计算或比对 MD5/SHA-256；历史字段仅兼容保留。来源身份改用 DOI/PID、版本、文件 ID/文件名、发布日期和访问时间；工程漂移改用字节数、成员清单、schema、行数和关键统计 | 主任务后续实现时删除哈希计算/校验路径，不因历史哈希字段失效而阻断下载、解析、路由或验收 |
| 2026-08-06 18:35 | TPDC 中国山地土壤官方文件契约与全量字段审计 | DOI `10.11888/Terre.tpdc.302620` 解析到固定 UUID，核实 metadata POST、文件清单 GET、file-ID POST 下载与 CC BY 4.0 code；固定 1,828,683-byte ZIP 及三个成员文件名/字节数。主表对账 1,314 条唯一 O/A/C 层记录、30 座山地、166 站点和 6,570 条 Cr/Cu/Ni/Pb/Zn 测定；As/Hg 缺失。坐标、母岩和土类逐行完整但 CRS 未声明；BD 补表能补 58 个缺失行，同时有六个非空 BD 冲突、五个坐标冲突和十个无匹配行。已写候选审计、目录和使用边界，没有提前计入十四个可执行来源 | 实现 V4-M0 profile 输出契约与 M1 sample/method/geology schema；随后用双值/冲突字段接入 TPDC adapter 和 30 条复核 |
| 2026-08-06 18:05 | AfSIS Phase I V2.0 非洲土壤工程闭环 | 从 World Agroforestry 官方 Dataverse 固定 3 份 original CSV/XLSX 的文件 ID、文件名、字节数与版本；解析 2,002 个唯一 SSN/RES.ID、18 个国家标签、51 个国家-站点对、1,876 个完整坐标对和 12,012 条六元素数值。逐元素附加 ICP-MS/ICP-OES、王水准全量基础、DL/QL、负数及低于限值标记；Pb 的 1,969 条正值低于 DL 被保留并降级解释。30 条复核样本覆盖全部国家标签、上下层、缺坐标、负数与阈值类别，后续已迁移为 Codex 结构化自动审计；48 条正值样板并入联合包，当时为 14 个 A/`normalized_analysis` 来源、736 条观测、89 个比较组、16 个工程异常候选 | 优先解析 TPDC 官方文件下载链路；并推进 GEMAS/NGSA 与 GEOROC Antarctica |
