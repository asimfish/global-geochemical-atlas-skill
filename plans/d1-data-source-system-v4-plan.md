...
1. **元素含量**：当前二十一个可执行数据集都能输出元素、原始数值、单位和来源定位；全量共 4,086,778 条目标测定，联合演示 1,144/1,144 具备这些字段。但演示只抽取可用记录，不能代表全量数据的缺失率。
2. **采样坐标**：联合演示 1,144 条均保留来源报告坐标，其中 808 条具有可进入地图的 canonical EPSG:4326 坐标；全量 771,322 个来源内去重样品中 766,402 个有来源报告坐标，731,521 个已规范为 canonical EPSG:4326。缺失坐标、未声明 CRS、未执行的显式转换和坐标范围仍分别保留，不由演示切片掩盖。
...
4. **分析方法**：联合演示中 896/1,144 有方法及 scope，其余 248 条有明确缺失原因。全量口径差异很大：GEMStat 只有 279,225/3,739,180（7.47%）方法明确；GEOTRACES 有 22,254/39,327（56.59%）能唯一关联到 BODC 方法记录；GEOROC precompiled 成员不提供 winning method，保持缺失而不猜测。
5. **样品类型**：二十一来源均已贯通 V4 语义。联合演示 1,144/1,144 有 `sample_type_raw + sample_type + mapping_status`；440 条土壤按层位/用途区分，408 条沉积物有环境类型，200 条水体有水体类型和分相。映射只来自来源 evidence 或注册常量，不从元素、单位或方法推断。
...
8. **自动采集**：下载、缓存、快照、适配器和来源定位链路已经存在；二十一来源发布最近成功获取/解析、来源版本、文件名、字节数、内容 SHA-256、字段清单、row-count、缓存/离线重放和版本漂移状态。

### 1.1 V4 内容寻址原则

- 原始文件、fixture、报告和交付包均记录 SHA-256；有 publisher checksum 时同时校验发布方摘要；
- 文件身份由 `source_id + DOI/PID + dataset_version + file_id/file_name + publisher release date + accessed_at + SHA-256` 共同绑定；
- 缓存命中、离线重放、回归测试与交付验收失败关闭，不能用同名文件或相同字节数代替内容一致性；
- 工程漂移同时检查响应状态、文件字节数、成员清单、字段集、类型、行数、目标元素数和关键分类计数；
- 哈希只证明所处理字节未漂移，不证明出版者、方法或测量值科学正确；科学可信性仍由出版者、版本、方法、单位、介质、QC、文献、人工复核和适用边界支撑。
...
用户提供的 304 条检查对应 2026-08-05 的五来源联合 fixture；736 条对应 M2 完成时的十四来源版本；M4 为十五来源、792 条，M5 为十八来源、944 条。当前主线保留原 USGS 三层样品后，M6 基线是二十一来源、1,144 条 fixture。这些数字都是各自时点的真实快照，不能混成同一时点的完整率。
...
| 自动采集 | 脚本化流水线和 snapshot manifest | 十四个 adapter、离线 fixture 与联合重建 | 增加内容哈希、版本/字段/行数漂移、缓存完整性和失败降级监控 |
...
下表统计的是当前 1,144 条版本固定联合演示，不是全量来源总体。

| 介质 | 演示记录 | 元素+值 | 坐标 | 来源/匹配地质单元 | 方法或缺失原因 | 样品类型 |
|---|---:|---:|---:|---:|---:|---:|
| 岩石 | 96 | 100% | 0% canonical | 0% | 100% | 100% |
| 土壤 | 440 | 100% | 80.0% canonical | 0% | 100% | 100% |
| 沉积物 | 408 | 100% | 74.5% canonical | 0% | 100% | 100% |
| 水体 | 200 | 100% | 76.0% canonical | 0% | 100% | 100% |
| 合计 | 1,144 | 100% | 70.6% canonical | 0% | 100% | 100% |

这里的“方法或缺失原因”不等于方法完整率：896 条有方法和 scope，248 条只有明确缺失原因。336 条来源报告坐标因 CRS 未声明、非 WGS84 datum 或尚未显式转换而不进入 canonical 地图。真正地质单元 0% 是清理错误语义后的诚实结果，不是数据被误删；地理上下文仍保留在专门字段中。
...
| 自动化健康 | 获取、解析、版本、SHA-256、schema、行数与离线重放可验证 | 漂移或下载失败时只降级到最后一个内容哈希验证通过的快照并报告陈旧度 |
...
12. adapter 必须检测版本、内容 SHA-256、文件字节数、成员清单、schema、行数和关键分类计数漂移，并能从最后一个验证通过的快照离线重放；
...
|---|---|---|---|
| 2026-08-07 18:45 | V4-M6 三个优先适配器与 21 来源联合验收 | 新增 GEOROC Antarctica 424 行/1,183 条岩石测定、TPDC 1,314 行/6,570 条土壤测定、GEMAS 4,131 个物理样品/8,262 个方法分离记录/53,703 条土壤测定。三者均完成 registry/catalog、adapter、V4 语义、全量 profile、内容寻址 snapshot/reconciliation、30 条待人工签署复核和离线 demo。21 来源全量为 4,086,778 条测定、771,322 个样品、766,402 个来源坐标样品、731,521 个 canonical 坐标样品、383,828 条最低可比测定和 8,268 行覆盖立方体；上游独立血缘按介质为岩石 1、土壤 6、沉积物 6、水体 4；1,144 条联合 fixture 拆成 156 个比较组，产生 27 个仅供工程验证的候选异常 | M6 仍需增加一条与 GEOROC 不同上游血缘的岩石来源；随后进入 M7 查询、地图、文档与交付自查 |
| 2026-08-07 17:57 | V4-M5 沉积物增强完成（后续已恢复内容寻址） | 新增 NGSA Hg 2,396 条、GSJ marine 34,334 条、PANGAEA Arabian Sea 162 条测定，三个来源均完成 adapter、全量 profile、30 条待人工签署复核和离线 demo。该阶段曾短暂移除哈希校验，主线合并时已由 SHA-256 内容寻址契约覆盖 | 执行 M6 并保持来源文件、fixture 与派生产物哈希链 |
| 2026-08-07 16:46 | 历史方案：取消哈希（已撤销） | 该方案与来源真实性门禁、离线复现和比赛证据链要求冲突，未进入最终主线 | 以本计划 1.1 的内容寻址原则为准 |
