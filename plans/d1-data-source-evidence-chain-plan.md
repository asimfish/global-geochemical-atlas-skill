...
- [ ] 实现 `scripts/download_data.py`。
- [ ] 下载时先写入 `.part` 临时文件，成功后原子重命名。
- [ ] 设置连接超时和读取超时。
- [ ] 仅对 `429`、`502`、`503`、`504` 等可恢复错误进行有限重试。
- [ ] 设置单文件最大下载体积。
- [ ] 检查 Content-Type，拒绝将 HTML 错误页当作数据文件。
- [ ] 计算并验证 SHA-256。
- [ ] 安全解压 ZIP，阻止路径穿越。
- [ ] 校验必要文件和必要字段。
- [ ] 记录最终 URL、ETag、Last-Modified、下载时间和文件 hash。
- [ ] 有效缓存存在时不重复联网。
- [ ] 来源版本或 hash 变化时停止，并要求更新注册表或明确确认。
