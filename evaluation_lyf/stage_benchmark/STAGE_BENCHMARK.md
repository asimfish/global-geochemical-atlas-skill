...
| D3 专业可视化 | 冻结 D1 manifest 与 D2 九项输出 | 离线交互地图及其标准数据库、来源、置信度、异常和复现配置 | 筛选、热力、组合、证据下钻、空白解释、专业边界和离线可复现 |
...
```text
evaluation_lyf/stage_benchmark/global-data/fixtures/raw
evaluation_lyf/stage_benchmark/expanded-data/fixtures/raw
evaluation_lyf/stage_benchmark/contracts/global-sources.json
evaluation_lyf/stage_benchmark/contracts/expanded-sources.json
```

### D2 的冻结输入（运行时由 D1 重建）

```text
<temporary-gold-run>/d1/d1_global_export.csv
<temporary-gold-run>/d1/d1_global_manifest.json
```

### D3 的冻结输入（运行时由参考 D2 重建）

```text
<temporary-gold-run>/d2
<temporary-gold-run>/d1/d1_global_manifest.json
```

上述临时产物由 `run_stage_benchmark.py` 从仓库内固定哈希的离线原始切片自动重建，不提交到
Git。D3 的输入会复制到本次 `--output-dir` 的 `d3/gold_input/`，因此报告不会引用运行结束后
失效的临时路径。也可使用 `--gold-run-dir` 显式复用已有参考运行。

参考 D3 成品由候选渲染器在本次输出目录的 `d3/candidate_output/` 生成。
...
必须输出九个 `d2-interface-v2` 文件。D2 不得下载新来源、重写来源证据或把候选异常解释为污染、
矿化或成因结论。
