...
def materialize_reference_gold(root: Path, *, include_d2: bool) -> tuple[Path, Path]:
    """Build the frozen D1/D2 handoff without relying on uncommitted run artifacts."""
    prepare_empty_directory(root)
    d1_dir = root / "d1"
    d1_execution = run_command(
        [
            sys.executable,
            str(REFERENCE_D1),
            "--core-fixture-dir",
    ...
    d2_dir = root / "d2"
    if include_d2:
        d2_execution = run_command(
            [
                sys.executable,
                str(REFERENCE_D2),
                "--input",
                str(d1_dir / "d1_global_export.csv"),
                "--output-dir",
                str(d2_dir),
            ]
        )
        if d2_execution["returncode"] != 0:
            raise ValueError(
                "could not materialize reference D2: "
                + (
                    d2_execution.get("stderr_tail")
                    or d2_execution.get("stdout_tail")
                    or "unknown error"
                )
            )
    return d1_dir, d2_dir


def validate_gold_inputs(d1_dir: Path, d2_dir: Path, *, include_d2: bool) -> None:
    required = [
        d1_dir / "d1_global_export.csv",
        d1_dir / "d1_global_manifest.json",
    ]
    if include_d2:
        required.extend(
            d2_dir / name
    ...
    if missing:
        raise ValueError("gold run is incomplete; missing: " + ", ".join(missing))
...
def build_d3_source_manifest(target: Path, gold_d1_dir: Path) -> dict[str, Any]:
...
def stage_d3_inputs(input_dir: Path, gold_d1_dir: Path, gold_d2_dir: Path) -> dict[str, Path]:
    ...
    for name in required:
        source = gold_d2_dir / name
        target = input_dir / name
        shutil.copy2(source, target)
        paths[name] = target
    source_manifest_path = input_dir / "source_manifest.json"
    build_d3_source_manifest(source_manifest_path, gold_d1_dir)
...
def run_d3_benchmark(
    ...
    input_paths = stage_d3_inputs(input_dir, gold_d1_dir, gold_d2_dir)
    candidate_dir = stage_root / "candidate_output"
    checks = Checks()
    checks.add(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional existing reference run containing d1/ and d2/; by default the required "
            "handoff is rebuilt from the hash-pinned offline fixtures in a temporary directory."
        ),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        needs_gold = "d1" in selected or "d3" in selected
        needs_gold_d2 = "d3" in selected
        with tempfile.TemporaryDirectory(prefix="geochem-stage-gold-") as temporary:
            if needs_gold:
                if args.gold_run_dir is None:
                    gold_d1_dir, gold_d2_dir = materialize_reference_gold(
                        Path(temporary), include_d2=needs_gold_d2
                    )
                else:
                    gold_root = args.gold_run_dir.resolve()
            ...
            if "d2" in selected:
                reports["d2"] = run_d2_benchmark(
                    args.output_dir, args.d2_script.resolve(), contract["d2"]
                )
            if "d3" in selected:
                reports["d3"] = run_d3_benchmark(
                    args.output_dir,
                    args.d3_script.resolve(),
                    contract["d3"],
                    gold_d1_dir,
                    gold_d2_dir,
                )
