...
```text
你是 D2 候选实现的维护 Agent。先阅读 evaluation_lyf/reference_implementation/d2/ 的接口、Schema、科学边界与现有实现。
禁止修改 evaluation_lyf/stage_benchmark/ 下的基准、冻结数据、契约和评分器；只修改候选 D2 代码。

候选脚本必须支持：
  python candidate_d2.py --input <csv> --output-dir <new-dir>
并输出 d2-interface-v2 规定的 9 个文件，成功 stdout 仅一行 JSON。
...
```bash
python3 evaluation_lyf/stage_benchmark/scripts/build_global_fixtures.py \
  --refresh \
...
`evaluation_lyf/stage_benchmark/DATA_SOURCE_CATALOG.md`。

## 防止 Agent “刷题”

正式横评时建议：

- 把 `evaluation_lyf/stage_benchmark/` 设为只读；
