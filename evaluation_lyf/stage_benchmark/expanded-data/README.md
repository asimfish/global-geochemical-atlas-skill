...
冻结基线统计为：44,536 个输入测定、44,199 个可标准化值、44,510 个可映射记录、535 个候选异常点和 276 个异常网格，所有 45 个阻断检查通过。5 个 review finding 是主动暴露的覆盖/元数据不足，不应伪装为已经解决。运行产物写入用户指定的仓库外输出目录，不随源码提交。
