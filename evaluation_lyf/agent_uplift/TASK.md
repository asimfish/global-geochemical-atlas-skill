...
唯一样品数和唯一坐标数；空白区域表示未覆盖，不表示元素不存在。全球产物还必须提供可拖动旋转、滚轮缩放
和点击样点的三维地球仪；区域产物应锁定请求范围，不用世界模板冒充区域专题图。

单元素地图须按同介质、测量基准、方法和单位的最大可比数据层显示 log10 含量色带，其他不可混合记录用中性色
区分。数据库页面不能只列 CSV，至少提供含量分布图、元素×介质覆盖矩阵和标准值/坐标/方法/地质/来源/QC
完整率。异常页面须同时给出 high/low 数量、候选空间密度图、统计区域和带表头的记录表；密度图只表达已观测
候选聚集，不预测未采样区浓度。

为便于外部 Chromium 对两臂执行同一黑盒验收，交互控件使用稳定语义 ID：二维地图 `map`、三维地球仪
`globe`、视图切换 `projectionMode`、浓度控制 `colorMode`、数据库三图
`databaseDistributionCanvas`/`databaseCoverageMatrix`/`databaseCompletenessChart`、异常密度图
`anomalyDensityCanvas`。这只是产品接口，不是评分答案；所有图形必须由本次输出数据动态生成。
