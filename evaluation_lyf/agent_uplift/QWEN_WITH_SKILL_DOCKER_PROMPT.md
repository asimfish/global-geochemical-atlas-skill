...
- commit: c59f2ab424480e5dc700d0628833c7cae9323bdf
- model: Qwen3.8-Max
- temperature=0
- 资源：2 CPU、4 GB 内存、256 PIDs、单次任务 900 秒
- 公开 scorer 最多三轮；只把锚点与新增来源的网络传输时间单独记录，来源规划、解析和处理仍计入任务执行耗时
...
3. 运行任务脚本时挂载 `work` 到 `/workspace`，工作目录设为 `/workspace/evaluation_lyf/agent_uplift`，使用 `--user "$(id -u):$(id -g)" --cpus 2 --memory 4g --pids-limit 256 --read-only --tmpfs /tmp:rw,nosuid,nodev --cap-drop ALL --security-opt no-new-privileges`，并把 `work/.agents/skills/global-geochemical-atlas` 只读挂载到容器内同一路径；固定锚点下载与 TASK 要求的有界多平台发现/采集阶段允许 bridge，冻结 `case_data/` 后的 D2、D3、评分和 clean rebuild 必须 `--network none`；
...
完整读取 `work/.agents/skills/global-geochemical-atlas/SKILL.md`，按其路由只读取本任务所需 references/scripts/assets，并记录实际使用文件。再读取 `TASK.md`、`public_case/sources.json`、`public_case/discovery_contract.json`、`public_case/prepare_case.py` 和 `public_case/score_submission.py`；分别记录这五个任务文件的 SHA-256，之后不得修改任何一个。

在 `experiment/with_skill_docker/` 中完成全流程：先准备 hash 固定锚点，再按冻结 global 请求执行有界多平台来源发现，把新增真实文件与 `discovered_manifest.json` 冻结到 `case_data/`；按 Skill 完成 D1、D2、D3 和全部交付。只处理四个固定地球化学锚点属于未完成 D1。最多进行三轮公开 scorer；每轮必须在容器工作目录中用 `python public_case/score_submission.py --case-dir experiment/with_skill_docker/case_data --submission-dir <本轮目录> --output <本轮score.json>` 评分，保存独立 score、日志、退出码和耗时，并检查 `source_truth.anchors.source_truth_score`、`source_truth.discovered.source_truth_score` 和 `source_truth.overall.source_truth_score`；不得用近似 DOI、空许可证、合成点或未绑定本地文件的哈希骗过来源核验；`agent_report.json` 必须包含字面字段 `"skill_used": true`；冻结采集输入后，使用 `--network none` 从保留的 `case_data/` 在 `clean_rebuild/` 实际执行 `run.sh` 并重新评分，不能复制最终 submission 冒充重建。

写出 `experiment/with_skill_docker/experiment_manifest.json`，至少记录 repository、commit、model、temperature、`skill_used=true`、Skill 文件与哈希、`runtime_mode=docker` 或真实 fallback、Docker version/image ID、资源、采集/离线网络阶段、scorer 与数据哈希、anchor_download_seconds、discovered_download_seconds、包含来源规划和处理的 execution_seconds、每轮分数、clean_rebuild 结果以及全部产物路径和 SHA-256。
