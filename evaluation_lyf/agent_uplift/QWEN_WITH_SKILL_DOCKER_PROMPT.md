# Qwen3.8-Max 有 Skill 组：Docker 正式运行入口
...
python3 evaluation_lyf/agent_uplift/export_candidate_bundle.py \
  --repo-root . --condition S0 --mode formal \
  --prompt evaluation_lyf/agent_uplift/candidate_prompts/DOCKER_S0.md \
  --output candidate_S0_docker
```
...
`runtime_mode=docker`，`skill_used=true`。把 bundle 只读挂载为候选初始目录；不得挂载完整仓库。原样提供
`AGENT_PROMPT.md`。S0 相比 B0 只额外包含 `.agents/skills/global-geochemical-atlas/`。候选不可见 `.git`、其他
Skill、score_submission.py、checker、rubric、gold、历史或另一臂。
...
只处理四个固定地球化学锚点属于未完成 D1；保存 Skill 文件/hash、发现 manifest、clean_rebuild、截图和全部
SHA-256，并核对 anchors、discovered、overall 三个 source_truth 分数。
