...
def safe_case_file(case_dir: Path, relative: Any, required_prefix: str | None = None) -> Path | None:
...
def case_source_contract(
    ...
    errors: list[str] = []
    discovery_contract_path = discovery_contract_path or public_contract_path.with_name("discovery_contract.json")
    try:
        contract_digest = hashlib.sha256(public_contract_path.read_bytes()).hexdigest()
        discovery_digest = hashlib.sha256(discovery_contract_path.read_bytes()).hexdigest()
    ...
    manifest_by_id = {item.get("id"): item for item in manifest.get("resources", []) if isinstance(item, dict)}
    ...
    for expected in public.get("resources", []):
        ...
        if local_path is None or not local_path.is_file():
            ...
            continue
        if local_path.stat().st_size != expected.get("bytes") or file_sha256(local_path) != expected.get("sha256"):
    ...
    if len(geochemical) != expected_count:
        errors.append(f"expected {expected_count} verified geochemical anchors, got {len(geochemical)}")
...
def discovered_source_contract(
    ...
    for index, item in enumerate(resources):
        ...
        if digest in by_hash:
            ...
            continue
        if not isinstance(media, list) or not media or any(value not in {"rock", "soil", "sediment", "water"} for value in media):
            errors.append(f"{label} has invalid media")
            continue
        if not all(is_https_url(item.get(field)) for field in ("authority_url", "download_url", "locator_prefix")):
            errors.append(f"{label} has invalid public HTTPS evidence URLs")
            continue
        local_path = safe_case_file(case_dir, item.get("file"), required_prefix=data_directory)
        if local_path is None or not local_path.is_file():
            errors.append(f"{label} file is missing or outside {data_directory}/")
            continue
        expected_bytes = item.get("bytes")
        if isinstance(expected_bytes, bool) or not isinstance(expected_bytes, int) or not 0 < expected_bytes <= maximum_bytes:
            errors.append(f"{label} has invalid bytes")
            continue
        total_bytes += expected_bytes
        if local_path.stat().st_size != expected_bytes or file_sha256(local_path) != digest:
            errors.append(f"{label} byte identity mismatch")
            continue
        by_hash[digest] = item
    if total_bytes > maximum_total_bytes:
        errors.append(f"discovered files exceed total byte cap: {total_bytes} > {maximum_total_bytes}")
    return by_hash, errors


def expected_medium_matches(expected: dict[str, Any], actual: str) -> bool:
    media = expected.get("media")
    return actual in media if isinstance(media, list) else actual == str(expected.get("medium") or "")
...
def source_truth_metrics(
    ...
    expected_ids = sorted(str(item["id"] if "id" in item else item["source_id"]) for item in resources_by_hash.values())
    if not rows:
        return {
            "hash_rate": 0.0,
            "metadata_rate": 0.0,
    ...
    for row in rows:
        ...
        if all((row.get(field) or "").strip() == str(value) for field, value in exact.items()) and expected_medium_matches(
            expected, (row.get("medium") or "").strip()
        ):
            metadata_ok += 1
        locator = (row.get("source_locator") or "").strip()
        if locator.startswith(str(expected["locator_prefix"])) and any(token in locator for token in LOCATOR_TOKENS):
            locator_ok += 1
...
def discovery_report_metrics(report: Any, contract: dict[str, Any], selected_ids: set[str]) -> dict[str, Any]:
    ...
    active_seconds = report.get("active_discovery_seconds") if isinstance(report, dict) else None
    valid = (
        ...
        and all(report.get(field) not in (None, "") for field in required_report_fields - {"remaining_candidates"})
        ...
        and 0 <= active_seconds <= contract["acquisition"]["discovery_planning_budget_seconds"]
    ...
    if isinstance(candidates, list):
        for item in candidates:
            if not isinstance(item, dict):
                valid = False
                continue
            required = ("source_id", "platform", "status", "authority_url", "query", "reason")
            if any(item.get(field) in (None, "") for field in required):
                valid = False
                continue
            source_id = str(item["source_id"])
            if source_id in candidate_ids or item["status"] not in allowed_statuses or not is_https_url(item["authority_url"]):
                valid = False
            ...
            if item["status"] == "selected":
                ...
                if item.get("marginal_contribution") in (None, "", []):
                    valid = False
    boundary = str(report.get("claim_boundary") or "") if isinstance(report, dict) else ""
    boundary_ok = any(token in boundary.casefold() for token in ("not exhaustive", "not complete", "非穷尽", "不代表完整", "覆盖空洞"))
...
def coverage_report_valid(report: Any, rows: list[dict[str, str]]) -> bool:
    if not isinstance(report, dict) or report.get("schema_version") != "qwen-uplift-coverage-report-v1":
        return False
    samples = {(row.get("source_id"), row.get("sample_id")) for row in rows if nonblank(row, "sample_id")}
    ...
    return (
        ...
        and any(token in boundary for token in ("not exhaustive", "not complete", "非穷尽", "不代表完整", "覆盖空洞"))
    )
...
def qc_tokens(row: dict[str, str]) -> set[str]:
    ...
    return {str(value).strip().upper() for value in parsed if str(value).strip()} if isinstance(parsed, list) else set()


def explicit_unit_disposition(row: dict[str, str]) -> bool:
    flags = qc_tokens(row)
    if any(any(token in flag for token in ("UNIT_INFERRED", "UNIT_GUESSED", "ASSUMED_UNIT")) for flag in flags):
        return False
    ...
    if censored or (row.get("value_qualifier") or "").strip().casefold() in {"<", ">", "nd", "bdl", "trace"}:
        return True
    allowed = ("UNSUPPORTED", "AMBIGUOUS", "MISSING_UNIT", "CENSORED", "NONDETECT", "TRACE")
...
def main() -> int:
    ...
    required = [
        "d1_raw.csv", "discovery_report.json", "coverage_report.json", "source_manifest.json",
        "record_evidence.jsonl", "geochemistry.csv", "qc_report.json", "confidence_report.json",
        "geology_report.json", "anomalies.geojson", "anomaly_report.json", "interactive_map.html",
        "samples.geojson", "source_confidence.json", "run.sh", "agent_report.md", "agent_report.json",
    ]
    ...
    anchor_resources, anchor_errors = case_source_contract(args.case_dir, public_case / "sources.json", discovery_contract_path)
    discovered_resources, discovered_errors = discovered_source_contract(args.case_dir, discovery_contract_path)
    anchor_rows = [row for row in d1 if (row.get("file_sha256") or "").strip() in anchor_resources]
    discovered_rows = [row for row in d1 if (row.get("file_sha256") or "").strip() in discovered_resources]
    ...
    check(
        "d1_anchor_coverage_exact", 2,
        anchor_truth["covered_resource_ids"] == anchor_truth["expected_resource_ids"],
        {"covered": anchor_truth["covered_resource_ids"], "expected": anchor_truth["expected_resource_ids"]},
    )
    check("d1_anchor_file_hash_exact", 2, anchor_truth["hash_rate"] == 1, anchor_truth["hash_rate"])
    check("d1_anchor_authority_metadata_exact", 2, anchor_truth["metadata_rate"] == 1, anchor_truth["metadata_rate"])
    check("d1_anchor_source_locator_exact", 2, anchor_truth["locator_rate"] == 1, anchor_truth["locator_rate"])
    ...
    discovery_metrics = discovery_report_metrics(discovery_report, discovery_contract, selected_ids)
    ...
    covered_discovered_resources = [item for item in discovered_resources.values() if item["source_id"] in covered_discovered_ids]
    platforms = {str(item["platform"]) for item in anchor_resources.values() if str(item.get("id")) in covered_anchor_ids}
    platforms.update(str(item["platform"]) for item in covered_discovered_resources)
    discovered_samples = {(row.get("source_id"), row.get("sample_id")) for row in discovered_rows if nonblank(row, "sample_id")}
    breadth_actual = {
        ...
        "verified_geochemical_source_count": len(covered_anchor_ids) + len(covered_discovered_ids),
    ...
    check(
        "d1_requested_media_element_coverage", 2, coverage_ok,
        {"media": sorted(actual_media), "elements": sorted(actual_elements), "additional_media": sorted(discovered_media)},
    )
    provenance_rate = sum(
        all(nonblank(row, field) for field in ("source_id", "source_locator", "file_sha256", "license")) for row in d1
    ) / len(d1) if d1 else 0
    ...
    check("d1_record_provenance", 2, provenance_rate == 1 and evidence_valid and evidence_ids == d1_ids, {
        "provenance_rate": provenance_rate, "evidence_records": len(evidence_ids), "d1_records": len(d1_ids)
    })
    ...
    check("d1_all_source_truth", 1, overall_truth["source_truth_score"] == 100, overall_truth)
    ...
    molar_ok = bool(molar) and all((row.get("normalized_unit") or "").casefold() in {"ug/l", "µg/l"} for row in molar)
    ...
    check("d2_atomic_weight_spot", 3, math.isfinite(molar_value) and abs(molar_value - 1.2216444276) < 1e-9, molar_value)
    disposition_rate = sum(explicit_unit_disposition(row) for row in d2) / len(d2) if d2 else 0
    check(
        "d2_units_have_explicit_disposition", 4, disposition_rate == 1, disposition_rate,
    ...
    anomaly_features = anomaly_geo.get("features", []) if isinstance(anomaly_geo, dict) else []
    ...
    check("repro_entrypoint", 5, len(run_text.strip()) > 20 and "case_data" in run_text, None)
    ...
    check("reported_failure_boundaries", 5, len(limits) >= 100 and any(token in limits.casefold() for token in ("not exhaustive", "不完整", "覆盖空洞", "partial_success")), len(limits))
