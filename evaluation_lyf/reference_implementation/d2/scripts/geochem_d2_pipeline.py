...
def species_conversion_factor(
    ...
    if species_key is None or species_key == analyte.upper():
        return 1.0, None
...
def parse_measurement(
    ...
    censoring_limit = (
        number if qualifier in {"lt", "le", "gt", "ge"} else detection_limit
    )
...
def score_confidence(record: Mapping[str, Any]) -> dict[str, float | str]:
    ...
    if record["license"] is None:
        source -= 0.10
    if record["source_file"] is not None and record["file_sha256"] is None:
        source -= 0.05
...
def normalize_row(
    ...
    original_value, qualifier, censoring_limit, detection_limit, missing_reason = (
        parse_measurement(row, flags)
    )
    unit_factor, normalized_unit = conversion_for(
        medium, row.get("unit"), analyte, flags
    )
        ...
        if (
            ...
            if qualifier in {"bdl", "nd"} and limit_unit is not None:
                ...
                limit_factor, limit_target = conversion_for(
                    medium, limit_unit, analyte, limit_flags
                )
                if (
    ...
    record: dict[str, Any] = {
        ...
        "source_qualifier_raw": blank_to_none(row.get("source_qualifier_raw")),
        ...
        "coordinate_uncertainty_status": blank_to_none(
            row.get("coordinate_uncertainty_status")
        ),
        "coordinate_uncertainty_basis": blank_to_none(
            row.get("coordinate_uncertainty_basis")
        ),
        "coordinate_resolution_m": parse_optional_float(
            row.get("coordinate_resolution_m")
        ),
        "coordinate_resolution_basis": blank_to_none(
            row.get("coordinate_resolution_basis")
        ),
        "measurement_uncertainty": parse_optional_float(
            row.get("measurement_uncertainty")
        ),
        "measurement_uncertainty_unit": blank_to_none(
            row.get("measurement_uncertainty_unit")
        ),
        "measurement_uncertainty_basis": blank_to_none(
            row.get("measurement_uncertainty_basis")
        ),
...
class GlimGrid:
    """Hash-independent reader for the official GLiM 0.5 degree Arc/ASCII grid."""
...
def apply_spatial_geology(
    records: Sequence[dict[str, Any]], grid: GlimGrid | None
) -> None:
    for record in records:
        if grid is None:
            continue
        ...
        if medium == "water":
            ...
            record["spatial_geology_reason"] = (
                "Land surface lithology is not assigned to water observations."
            )
            continue
        if medium == "sediment" and "marine" in material:
            record["spatial_geology_status"] = "not_applicable_marine_sediment"
            record["spatial_geology_reason"] = (
                "Land surface lithology is not assigned to marine sediment."
            )
            continue
        ...
        if latitude is None or longitude is None:
            ...
            record["spatial_geology_reason"] = (
                "No valid canonical point was available for the spatial join."
            )
            continue
        ...
        record["spatial_geology_reason"] = (
            "Point contained by the deterministic 0.5 degree GLiM raster cell."
        )
...
def build_geology_report(
    ...
    eligible = sum(
        status not in {"not_applicable_water", "not_applicable_marine_sediment"}
        for status in (str(record["spatial_geology_status"]) for record in records)
    )
    matched = statuses["matched"]
    water_with_unit = sum(
        record["medium"] == "water" and record.get("spatial_geologic_unit") is not None
        for record in records
    )
    return {
...
def build_samples_geojson(records: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    features: list[dict[str, Any]] = []
        ...
        features.append(
            {
                ...
                "properties": {
                    ...
                    "sample_id": record["sample_id"],
                    "element_or_analyte": record["element_or_analyte"],
                    "analyte_reported": record["analyte_reported"],
                    "species_or_oxide": record["species_or_oxide"],
                    "medium": record["medium"],
                    "measurement_basis": record["measurement_basis"],
                    "original_value_raw": record["original_value_raw"],
                    "source_qualifier_raw": record["source_qualifier_raw"],
                    "original_unit": record["original_unit"],
                    "value_qualifier": record["value_qualifier"],
                    "censored": record["censored"],
                    "normalized_value": record["normalized_value"],
                    "normalized_unit": record["normalized_unit"],
                    "normalized_censoring_limit": record["normalized_censoring_limit"],
                    "geologic_unit": record["geologic_unit"],
                    "lithology": record["lithology"],
                    "spatial_geology_status": record["spatial_geology_status"],
                    "spatial_geologic_unit": record["spatial_geologic_unit"],
                    "spatial_geologic_unit_id": record["spatial_geologic_unit_id"],
                    "spatial_geology_version": record["spatial_geology_version"],
                    "method_family": record["method_family"],
                    "analytical_method": record["analytical_method"],
                    "qc_flags": record["qc_flags"],
                    "operational_confidence": record["operational_confidence"],
                    "source_id": record["source_id"],
                    "source_locator": record["source_locator"],
...
def build_map_spec(run_metadata: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "interface_version": INTERFACE_VERSION,
        ...
        "filters": [
            ...
            "spatial_geologic_unit",
            "method_family",
            "operational_confidence.band",
            "qc_flags",
        ],
        "layers": [
            {"id": "observations", "source": "samples", "required": True},
            {
                "id": "censored_observations",
                "source": "samples",
                "filter": "censored == true",
                "required": True,
            },
            {"id": "candidate_anomalies", "source": "anomalies", "required": True},
            {"id": "sample_density", "source": "samples", "required": False},
        ],
        "no_data_rules": {
            "null_normalized_value": "Do not render as zero or include in a continuous color scale.",
            "missing_coordinate": "Keep in the table/QC panel; do not place at (0, 0).",
            "censored_value": "Use a distinct symbol and display the qualifier and censoring limit.",
        },
        "required_disclaimers": [
            "Candidate anomalies are relative to declared background groups and have no causal interpretation.",
            "Operational confidence is workflow usability, not probability or analytical accuracy.",
            "Coverage gaps must remain visible; do not imply global completeness or interpolate by default.",
        ],
    }
...
def build_qc_report(
    ...
    numeric_or_limit = sum(
        record["original_value"] is not None or record["detection_limit"] is not None
        for record in records
    )
    standardized = sum(
...
def write_csv(path: Path, records: Sequence[Mapping[str, Any]]) -> None:
    ...
    with tempfile.NamedTemporaryFile(
        ...
        writer = csv.DictWriter(
            handle, fieldnames=SCHEMA_COLUMNS, extrasaction="ignore"
        )
        writer.writeheader()
        for record in records:
            row = dict(record)
...
def build_run_manifest(
    ...
    quality_flags = sorted(qc_report["flag_counts"])
    return {
        "manifest_version": "d2-run-manifest-v1",
        "interface_version": INTERFACE_VERSION,
        "pipeline_version": PIPELINE_VERSION,
        "status": "success_with_quality_flags" if quality_flags else "success",
        "deterministic": True,
        "timestamps_omitted_for_determinism": True,
        "input": {
            "path": input_path.name,
            "sha256": run_metadata["input_sha256"],
            "row_count": run_metadata["input_row_count"],
            "schema_map_sha256": run_metadata["schema_map_sha256"],
        },
        "configuration": run_metadata["configuration"],
        "record_counts": qc_report["record_counts"],
        "quality_flag_types": quality_flags,
        "contracts": {
            "canonical_record_schema": "urn:ai4s:geochemistry-record:d2-v2",
            "map_interface": "d2-map-spec-v1",
        },
        "outputs": output_entries,
        "scientific_scope": "standardization_qc_and_screening_only",
        "limitations": [
            "Candidate anomalies do not establish cause, pollution, or mineralization.",
            "Coverage is limited to the supplied input and must not be described as globally complete.",
        ],
    }
...
def run_pipeline(
    ...
    geology_grid = (
        GlimGrid(geology_grid_path) if geology_grid_path is not None else None
    )
    ...
    config = {
        ...
        "geology_grid_sha256": sha256_file(geology_grid_path)
        if geology_grid_path is not None
        else None,
        "geology_join_version": GEOLOGY_JOIN_VERSION
        if geology_grid_path is not None
        else None,
    }
    ...
    samples_geojson = build_samples_geojson(records)
    map_spec = build_map_spec(run_metadata)
    geology_report = build_geology_report(records, geology_grid_path, run_metadata)

    outputs = {
        "database": output_dir / "geochemistry.csv",
        "samples": output_dir / "samples.geojson",
        ...
        "map_spec": output_dir / "map_spec.json",
    }
    write_csv(outputs["database"], records)
    atomic_write_text(outputs["samples"], json_text(samples_geojson))
    ...
    atomic_write_text(outputs["map_spec"], json_text(map_spec))
    manifest_path = output_dir / "run_manifest.json"
    manifest = build_run_manifest(input_path, outputs, run_metadata, qc_report)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory for deterministic D2 interface outputs",
    )
    parser.add_argument(
        "--schema-map",
        type=Path,
    ...
    parser.add_argument(
        "--geology-grid",
        type=Path,
        help="Optional hash-pinned PANGAEA.788537 ZIP for versioned GLiM 0.5 degree point-in-cell matching",
    )
    parser.add_argument(
        "--group-by",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Minimum positive uncensored records per group",
    )
    parser.add_argument(
