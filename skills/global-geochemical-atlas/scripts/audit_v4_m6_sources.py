...
def build_source(source_id: str, medium: str) -> None:
    ...
    for source_file in manifest["source_files"]:
        ...
        if not isinstance(sha256, str) or len(sha256) != 64:
            raise ValueError(f"{source_id} source file lacks a pinned SHA-256: {source_file['filename']}")
        files.append({
            "file_id": next(
                (
                    item.get("file_id")
    ...
    for index, (row, evidence_row) in enumerate(zip(rows[:30], evidence[:30], strict=True), 1):
        ...
        review_records.append({
            ...
            "source_file_sha256": evidence_row.get("source_file_sha256") or files[0]["sha256"],
            "selection_reasons": ["source_order_prefix", "target_observation_mapping"],
...
def main() -> int:
    ...
    print(json.dumps({"status": "PASS", "sources": sorted(SOURCES), "content_hashes_used": True}))
    return 0
