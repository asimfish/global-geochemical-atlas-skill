...
def _merge_cube(
    ...
    if replacement_source_id:
        replaced_rows = [
            row for row in base_rows if row["source_id"] == replacement_source_id
        ]
        base_rows = [
            row for row in base_rows if row["source_id"] != replacement_source_id
        ]
    paths_and_rows = [
        (base, base_rows),
        *((path, _read_cube(path)) for path in expansions),
    ]
    if replacement_source_id:
        ...
        if {row["source_id"] for row in replacement_rows} != {replacement_source_id}:
            raise MergeError(
                "replacement cube must contain exactly the replacement source"
            )
    ...
    for path, current in paths_and_rows:
        ...
        if overlap:
            raise MergeError(
                f"coverage cube source overlap: {', '.join(sorted(overlap))}"
            )
    ...
    return (
        sorted(
            ...
            key=lambda row: tuple(
                row[column] for column in build_v4_full_profiles.CUBE_COLUMNS[:8]
            ),
        ),
        replaced_rows,
    )
...
def _replace_source_balance(
    ...
    if not old_rows:
        raise MergeError(
            f"replacement source is absent from the base cube: {source_id}"
        )
    ...
    if len(old_elements) != 1:
        raise MergeError(
            "automatic replacement currently requires a single-element base source"
        )
    for field in old_total:
        target_medium[field] = (
            int(target_medium[field])
            - old_total[field]
            + int(replacement_medium[field])
        )
    target_medium["source_ids"] = sorted(set(target_medium["source_ids"]) | {source_id})
    target_medium["lineage_ids"] = sorted(
        set(target_medium["lineage_ids"])
        | set(replacement_medium.get("lineage_ids", []))
    )
    target_medium["independent_lineage_count"] = len(target_medium["lineage_ids"])
    if reported_union:
        target_medium["covered_spatial_cells"] = int(
    ...
    for key, new_value in replacement.get("medium_elements", {}).items():
        ...
        if reported_union and key in reported_union.get("medium_elements", {}):
            union_cells = int(
                reported_union["medium_elements"][key]["covered_spatial_cells"]
            )
...
def _merge_metric(
    ...
    source_ids = sorted(
        {value for part in parts for value in part.get("source_ids", [])}
    )
    lineage_ids = sorted(
    ...
    if reported_union is not None:
        ...
        if reported_cells < canonical_base_cells:
            raise MergeError(
                "reported spatial union cannot be smaller than the canonical baseline"
            )
        ...
        if (
            reported_cells == canonical_base_cells
            and canonical_expansion_max > canonical_base_cells
        ):
            raise MergeError(
                "expansion cells cannot fit inside the verified baseline union"
            )
    else:
        reported_cells = max(
            int(part.get("reported_covered_spatial_cells", 0)) for part in parts
        )
    canonical_cells = max(int(part.get("covered_spatial_cells", 0)) for part in parts)
    if (
        reported_union is not None
        and int(reported_union["covered_spatial_cells"]) == canonical_base_cells
    ):
    ...
    return {
        ...
        "distinct_sample_count": sum(
            int(part["distinct_sample_count"]) for part in parts
        ),
        ...
        "reported_coordinate_sample_count": sum(
            int(
                part.get(
                    "reported_coordinate_sample_count",
                    part.get("valid_coordinate_sample_count", 0),
                )
            )
            for part in parts
        ),
        "valid_coordinate_sample_count": sum(
            int(part["valid_coordinate_sample_count"]) for part in parts
        ),
        "comparable_observation_count": sum(
            int(part["comparable_observation_count"]) for part in parts
        ),
...
def _profile_summary(profile_dir: Path, source_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "reported_coordinate_sample_count": int(
            spatial["reported_coordinate_sample_count"]
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--registry", type=Path, default=build_v4_full_profiles.DEFAULT_REGISTRY
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        reported_union = (
            _load_json(args.reported_union_balance)
            if args.reported_union_balance
            else None
        )
        ...
        for row in rows:
            ...
            totals["comparable_observation_count"] += int(
                row["comparable_observation_count"]
            )
        sources = []
        for source_id in source_ids:
            source = _profile_summary(args.profile_dir, source_id)
            source["comparable_observation_count"] = cube_totals[source_id][
                "comparable_observation_count"
            ]
        ...
        summary = {
            ...
            "distinct_sample_count": sum(
                item["distinct_sample_count"] for item in sources
            ),
            "reported_coordinate_sample_count": sum(
                item["reported_coordinate_sample_count"] for item in sources
            ),
            "valid_coordinate_sample_count": sum(
                item["valid_coordinate_sample_count"] for item in sources
            ),
            "comparable_observation_count": sum(
                item["comparable_observation_count"] for item in sources
            ),
            "covered_spatial_cells_source_sum": sum(
                item["covered_spatial_cells"] for item in sources
            ),
        ...
        balance_text = (
            json.dumps(balance, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
        )
        ...
        artifact_paths = sorted(
            ...
            + [
                path
                for source_id in source_ids
                for path in (args.profile_dir / source_id).glob("*.json")
            ],
            key=str,
        )
        manifest = {
