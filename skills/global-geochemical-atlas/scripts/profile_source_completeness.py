...
def build_profile(registry_path: Path, audit_dir: Path, demo_dir: Path) -> dict[str, Any]:
    ...
    for source_id in sorted(sources):
        ...
        full = _full_population(*(audit_item or (None, None)))
        profiles[source_id] = {
            "source_id": source_id,
            ...
            "demo_fixture": demo,
            "usage_rights": _rights(source),
            "automation_health": _automation(source, demo),
        }
    audited = sum(value["full_population"]["audit_status"] == "audited_snapshot" for value in profiles.values())
    ...
    missing_audits = sorted(
        ...
        if value["full_population"]["audit_status"] == "not_measured"
    )
    findings = [
        {
            "severity": "high",
            "code": "FULL_FIELD_COMPLETENESS_NOT_UNIFORM",
            "affected_sources": len(profiles),
            "detail": "No source currently has a uniform full-population numerator/denominator for every V4 field.",
    ...
    return {
        ...
        "population_definitions": {
            "full_population": "Counts explicitly observed in a checked-in candidate audit over a verified source snapshot.",
        ...
        "findings": findings,
        "claim_boundary": (
            "This report measures evidence availability and demo-field completeness. It does not validate scientific "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        profile = build_profile(args.registry, args.audit_dir, args.demo_dir)
