...
def _profile_one(
    ...
    for element in sorted(elements):
        ...
        element_details[element] = {
            ...
            "reported_coordinate_sample_count": reported_count,
        ...
        cube_rows.append(
            {
                ...
                "reported_coordinate_sample_count": cube_reported_count,
...
def build(
    ...
    if selected_source_ids:
        unknown = sorted(set(selected_source_ids) - set(sources))
        if unknown:
            raise FullProfileError(f"unknown source IDs: {', '.join(unknown)}")
        selected = {source_id: sources[source_id] for source_id in selected_source_ids}
    else:
        selected = sources
    profiles: dict[str, dict[str, Any]] = {}
    cube_rows: list[dict[str, Any]] = []
    source_summaries = []
    for source_id, entry in sorted(selected.items()):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        summary, profiles, cube_rows, balance = build(
            args.registry,
            args.cache_root,
            selected_source_ids,
        )
