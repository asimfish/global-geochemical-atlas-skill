...
def _load(path: Path) -> dict[str, Any]:
    ...
    if validation["status"] != "PASS":
        raise ExportError("archive bundle validation failed: " + "; ".join(validation["errors"]))
    return value


def _index(items: Sequence[Mapping[str, Any]], field: str) -> dict[str, Mapping[str, Any]]:
...
def export_rows(bundle: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    for observation in bundle["observations"]:
        ...
        publication_id = str(publication_ids[0]) if isinstance(publication_ids, list) and publication_ids else ""
        ...
        row.update(
            {
                ...
                "measurement_basis": str(observation.get("measurement_basis_raw") or ""),
                "value_qualifier": str(observation.get("value_qualifier") or ""),
                "detection_limit": str(observation.get("detection_limit_raw") or ""),
                "detection_limit_unit": str(observation.get("detection_limit_unit_raw") or ""),
                "latitude": str(location.get("latitude") if location.get("latitude") is not None else ""),
                "longitude": str(location.get("longitude") if location.get("longitude") is not None else ""),
                "source_crs": str(location.get("source_crs") or ""),
                "coordinate_uncertainty_m": str(
                    location.get("coordinate_uncertainty_m")
                    if location.get("coordinate_uncertainty_m") is not None else ""
                ),
                # Legacy geologic_unit remains blank. V4 never copies geographic context into it.
                "geologic_unit": "",
                "analytical_method": str(method.get("technique_raw") or method.get("method_code_raw") or ""),
                "digestion_or_extraction": str(method.get("digestion_or_extraction_raw") or ""),
                ...
                "sample_depth_min_m": str(location.get("depth_min_m") if location.get("depth_min_m") is not None else ""),
                "sample_depth_max_m": str(location.get("depth_max_m") if location.get("depth_max_m") is not None else ""),
                "grain_fraction": str(sample.get("grain_fraction_raw") or ""),
                "sample_type_raw": str(sample.get("sample_type_raw") or ""),
                "sample_type": sample_type,
                "sample_type_mapping_status": str(sample.get("sample_type_mapping_status") or ""),
                "sample_type_missing_reason": _missing(sample, "sample_type") if not sample_type else "",
                "geographic_context_raw": str(sample.get("geographic_context_raw") or location.get("place_raw") or ""),
                ...
                "quantitation_limit": str(method.get("quantitation_limit_raw") or "").split(" ", 1)[0],
                "quantitation_limit_unit": (
                    str(method.get("quantitation_limit_raw") or "").split(" ", 1)[1]
                    if " " in str(method.get("quantitation_limit_raw") or "") else ""
                ),
                ...
                "method_missing_reason": _missing(method, "technique_raw") if not method.get("technique_raw") else "",
                ...
                "citation_assignment_basis": "method_publication_relation" if publication_id else "dataset_metadata",
                "citation_resolution_status": "resolved" if publication_id else "not_reported",
                ...
                "attribution_required": "false" if dataset.get("license_id") == "CC0-1.0" else "true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(json.dumps({"status": "PASS", "record_count": len(rows), "output": str(args.output)}, ensure_ascii=False))
    return 0
