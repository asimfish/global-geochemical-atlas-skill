...
<main class="layout">
  <section class="card">
    <div class="controls">
      ...
      <div class="control"><label for="sampleType">样品类型</label><select id="sampleType"></select></div>
      <div class="control"><label for="methodScope">方法范围</label><select id="methodScope"></select></div>
...
fillSelect("element",unique("element"),"全部元素");fillSelect("medium",unique("medium"),"全部介质");fillSelect("sampleType",unique("sample_type"),"全部样品类型");fillSelect("methodScope",unique("method_scope"),"全部方法范围");fillSelect("confidence",unique("confidence_band"),"全部置信度");
...
function filtered(){return samples.filter(s=>(!$("element").value||s.element===$("element").value)&&(!$("medium").value||s.medium===$("medium").value)&&(!$("sampleType").value||s.sample_type===$("sampleType").value)&&(!$("methodScope").value||s.method_scope===$("methodScope").value)&&(!$("confidence").value||s.confidence_band===$("confidence").value)&&(!$("anomalyOnly").checked||s.candidate_anomaly))}
...
function showTip(event,r){tip.replaceChildren();const lines=[`${r.element} · ${r.value??"删失/缺失"} ${r.unit??""}`,`记录: ${r.record_id}`,`样品: ${r.sample_id??"未提供"}`,`介质/样品类型: ${r.medium} / ${r.sample_type??"未提供"}`,`分相/层位: ${r.water_fraction??r.sediment_environment??r.soil_horizon??"未提供"}`,`basis: ${r.measurement_basis??"未提供"}`,`来源地质/匹配地质: ${r.geologic_unit_raw??"未提供"} / ${r.matched_geologic_unit??"未提供"}`,`方法/范围: ${r.analytical_method??"未提供"} / ${r.method_scope??"未提供"}`,`置信度: ${r.confidence_band} (${r.confidence_overall??"-"})`,`QC: ${(r.qc_flags||[]).join(", ")||"无 flag"}`,`来源: ${r.source_id??"未提供"}`,`定位: ${r.source_locator??"未提供"}`];lines.forEach((line,i)=>{const d=document.createElement("div");d.textContent=line;if(i===0){const b=document.createElement("strong");b.textContent=line;d.replaceChildren(b)}if(i>=9)d.className="muted";tip.append(d)});tip.style.display="block";const box=wrap.getBoundingClientRect();tip.style.left=Math.min(event.clientX-box.left+12,box.width-340)+"px";tip.style.top=Math.max(8,event.clientY-box.top-20)+"px"}
...
for(const id of["element","medium","sampleType","methodScope","confidence","mode","anomalyOnly"])$(id).addEventListener("change",render);
