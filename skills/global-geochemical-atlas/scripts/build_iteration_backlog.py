...
def issue(
    ...
    *,
    ...
    record_id = (
        text(row, "record_id") or text(row, "source_record_id") or "unknown-record"
    )
    return {
...
def issues_for_row(row: Mapping[str, str]) -> list[dict[str, str]]:
    ...
    latitude, longitude = (
        finite_number(text(row, "latitude")),
        finite_number(text(row, "longitude")),
    )
    if (
        latitude is None
    ...
    if not sample_type:
        ...
        found.append(
            issue(
                ...
                code="SAMPLE_TYPE_MAPPING_REVIEW",
                field="sample_type_mapping_status",
                detail="样品类型映射不是明确的精确/来源直报状态。",
                action="复核映射表和原始术语；无法确定时保持 unknown，不做推测。",
                observed=mapping_status,
            )
        )
    analytical_method = text(row, "analytical_method")
    if not analytical_method or analytical_method.lower() in {
        "unknown",
        "not provided",
        "未提供",
    }:
        found.append(
            issue(
                ...
                severity="warning",
                status="action_required",
                code="ANALYTICAL_METHOD_MISSING",
    ...
    if not text(row, "matched_geologic_unit") and not text(row, "geologic_unit"):
        found.append(
            issue(
                ...
                severity="info",
                status="review_required",
                code="GEOLOGIC_CONTEXT_MISSING",
                field="matched_geologic_unit",
    ...
    if censored:
        found.append(
            issue(
                ...
                owner="D2",
                severity="info",
                status="scientific_limit",
                code="CENSORED_OBSERVATION",
                field="censored",
                detail="这是检出限/定量限约束的删失观测，不等同于零，也不自动视为采集失败。",
                action="保留原始 qualifier 与 censoring limit；默认从普通相关和异常筛查中排除，或使用明确的删失统计模型。",
                observed=text(row, "source_qualifier_raw")
                or text(row, "value_qualifier"),
                auto_recheck=False,
            )
        )
    ...
    if (
        ...
        found.append(
            issue(
                ...
                severity="warning",
                status="review_required",
                code="LOW_OPERATIONAL_CONFIDENCE",
                field="operational_confidence",
        ...
        if flag.lower().startswith(("error", "invalid", "unsupported")):
            found.append(
                issue(
                    ...
                    severity="error",
                    status="action_required",
                    code="QC_ERROR_REVIEW",
                    field="qc_flags",
                    detail=f"记录含错误级或不可支持的 QC 标志：{flag}",
                    action="按 QC 规则修复源字段或映射；修复前保持排除，不要删除原始记录。",
                    observed=flag,
                )
            )
    return found
...
def build(database: Path, output: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "owner_counts": dict(
            sorted(Counter(item["stage_owner"] for item in items).items())
        ),
        "issue_counts": dict(
            sorted(Counter(item["issue_code"] for item in items).items())
        ),
    }
...
def load(path: Path, preview_limit: int = 500) -> dict[str, Any]:
    ...
    return {
        ...
        "owner_counts": dict(
            sorted(Counter(item.get("stage_owner", "") for item in items).items())
        ),
        "issue_counts": dict(
            sorted(Counter(item.get("issue_code", "") for item in items).items())
        ),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a D1/D2 evidence and processing iteration backlog."
    )
    ...
    print(
        json.dumps(
            build(args.database, args.output), ensure_ascii=False, sort_keys=True
        )
    )
    return 0
