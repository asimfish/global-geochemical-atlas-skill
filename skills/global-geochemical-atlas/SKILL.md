...
autopilot 确定性完成：冻结请求（抽取证据写 `freeze_report.json`，题面候选全集进入请求、最低线只作验收）→ `task_router.py` 规划 → 执行与全部验证 → 对抗审计 → 断言台账 → 写 `executor_compliance_report.json`。终态机读：`AUTOPILOT_STATE=DONE|CONTINUE_REQUIRED|NEEDS_HUMAN_REVIEW|FAILED`。`CONTINUE_REQUIRED` 时唯一正确动作是执行报告中的 `next_command`（`--continue` 消费修复队列），不得改写为总结交差；最终回复的数字必须逐字取自报告的 `final_answer_facts`，`required_scope_notes` 与 `claims.scoped_claims` 中的范围句必须随对应数字一起出现。先用 `python scripts/claim_ledger.py --run-dir OUT --render-answer NEW_ANSWER.md` 生成规范证据附录，再用 `--check-answer NEW_ANSWER.md` 重新核验。只有原样保留的规范断言行获得 `answer_bound`；自由文本、改数值/语义/范围/哈希或附加解释均失败关闭。补充说明另行审查，不得把附录通过宣称为全文已认证。autopilot 不豁免任何门禁，手工路径与它执行同一套契约。
...
5. 论文与图件：每篇先交 2–5 条 claim-bound contribution map，再写 Results → Methods → Discussion → Introduction → Abstract → Title。写作与绘图波次开启时，controller 把**论文工具包** `paper_kit/`（契约 `gga-paper-v1`：固定样式 `gga-paper.sty`、手稿骨架 `manuscript-template.tex`、由冻结声明登记簿生成的 `claims.tex`、由已验证文献种子化的 `references.bib` 与 `seeded_reference_list.json`）和**图件工具包** `figure_kit/`（契约 `gga-figure-kit-v1`：零依赖 SVG 绘图库 `paper_figures.py`、SVG→同尺寸 PDF/288 dpi PNG 渲染器 `render_figure.py`、`publication_lint.py` 与离线 Natural Earth 海岸线/国界/中国省界底图）按哈希绑定进 packet；手稿必须从骨架起笔、加载 `gga-paper`、`\input{claims}`、所有数字只通过 `\claimref{claim_id}` 引用登记声明（禁止行内 `\claim{...}` 标签与正文 `\texttt` 标识符）、图件用 `\includegraphics[width=\linewidth]` 全宽放在正文参考文献之前并有 caption/label/正文引用、以 `\printclaimledger` 结尾，PDF 必须由 TeX 引擎编译（≥4 页、首页含 Abstract、嵌入模板字体；浏览器/办公软件导出一律拒绝）；`section_manifest` 不作自证——paper spine 全部章节（Title/Abstract 除外）必须在源文件里以 `\section`/`\subsection` 标题真实存在，payload `claim_ids` 中每个 id 必须在正文出现为 `\claimref` 标记，每条图注至少 80 字符以陈述结论与编码；writer packet 同样绑定图件工具包，手稿内嵌图与 storyboard 使用同一绘图语言。手稿必须同时交付：typed `reference_list`（唯一 reference_id、题名、有序作者、年份、期刊/会议与一个可解析的 DOI/arXiv/官方 URL 标识符，且恰好等于源文件 `\cite` 的 bib key 集合）与 `typeset_manifest`（互不相同的 LaTeX 源文件、`bib_artifact` 与渲染 PDF artifact，其 section manifest 覆盖 paper spine 全部冻结章节）。实证主图至少覆盖 primary result、spatial pattern、robustness/external validation 三种角色；方法图可以保留，但不能替代数据主图。spatial pattern 图必须在空间上编码被主张的效应或实测值本身——数据可用性/覆盖度地图不满足该角色，a6 门直接判负（图件合同 `spatial_pattern_semantics` 已固化此语义）。每张图声明科学问题、claim IDs、视觉编码，并分别绑定真实 SVG/PDF/PNG 的路径与 SHA-256；至少一次 render→inspect→revise 必须留下非空发现与已实施修改，不能只交布尔自评；另须记录 `candidate_generation`（至少比较 2 个候选设计并列出被否方案）、`source_fidelity`（渲染图如何忠实于源数据）与一个可编辑源文件 artifact（不得以 PDF/PNG 渲染件充当）。提交时 controller 对写作/绘图两角色声明的每个 SVG/PNG/PDF 运行确定性排版 lint：矢量文字低于 4.5 pt 打印等效地板（未声明字号按 16 px 计）、图内超过 120 字符的说明/免责段落、栅格宽度低于 1000 px、无页对象或损坏的 PDF（含 PDF 1.5 对象流内的页对象）、非良构 SVG 一律在门口拒绝，不依赖自评；每张图的 PDF 页框与 PNG 像素框必须与 SVG 纵横比一致（2% 容差，裁切或留白导出判负），`spatial_pattern`/`coverage_and_gaps` 必须含 `data-gga-layer="basemap"` 底图层；质量合同同时固化排版词法规则（名词短语式节标题、行内三项以上枚举须结构化列表、图内字号层级），由 a5/a6 评审门执行。Discussion 必须含「Position within the current frontier」与「New questions generated」；Data and code availability 逐源列许可证；Agent disclosure 强制保留；结论一律不越过筛查证据。
