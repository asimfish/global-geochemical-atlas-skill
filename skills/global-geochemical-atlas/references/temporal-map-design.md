...
2d. **页面滚动与地图缩放分离**:滚轮只在「点击过地图(激活)」或按住 Ctrl/⌘ 时缩放;未激活时滚轮原样滚动页面,并在地图中央短暂显示提示「点击地图后滚轮缩放 · 或按住 Ctrl/⌘ 滚动」;指针离开地图或按 Esc 取消激活。主图的二维画布、三维地球仪与时序画布同一策略。
2e. **嵌入模式单滚动条**:时序文档被主图以 sandbox `srcdoc` iframe 嵌入(无 `allow-same-origin`,父页读不到子文档),因此子文档检测到 `window.self !== window.top` 时:`html/body overflow:hidden`,地图高度固定 600px(避免 vh 与 iframe 高度互相反馈),并用 `ResizeObserver` 通过 `postMessage({type:"gga-temporal-height",height})` 向父页上报内容高度;父页校验 `event.source === temporalFrame.contentWindow` 后把 iframe 高度设为内容高度并加 `auto-height`。结果是整页只有一个滚动条,不再出现「外部滚动/内部滚动判定冲突」。
2f. **回放默认全部元素**:初始模式探测「区域对比」时需要单元素,探测完若无可比网格而回落到「采样史回放」,元素筛选必须恢复为「全部元素」——否则页面会停在字母序第一个元素(如 As 80 条)上,让 7,000 多条其他元素的带时间记录看起来「没有数据」。`bestCompareElement()` 在无任何可比网格时退回带时间记录最多的元素,而非字母序第一个。
