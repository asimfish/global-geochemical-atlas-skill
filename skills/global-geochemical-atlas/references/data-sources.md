...
核对日期：2026-08-07。端点和使用条件可能变化；每次运行重新记录访问日期和实际响应。

> V4 政策：不再计算、读取、更新或校验 MD5、SHA-256 等内容哈希，也不用于准入、评分、缓存、测试或验收。当前追踪依据是 DOI/PID、版本、文件 ID/名称、发布/访问时间、字节数、成员清单、schema、行数和关键统计。

正常请求按区域、元素和介质只获取相关来源，并优先复用合约一致的增量缓存或小 fixture。完整 26 来源下载和全量数据库构建仅用于显式发布、覆盖审计或验收模式，不是科研使用前置条件。
...
- 校验：验证版本、28 个成员的文件名、大小、字段、行数和关键分类统计；
...
- 校验：记录官方 URL、版本、响应元数据、文件名、字节数、字段和行数；
...
- 校验：30,336 bytes；43 个源行、48 个元素字段、As/Cr/Cu/Ni/Pb/Zn 各 43 条；
...
- 校验：样点表 334,302 bytes，浓度表 1,198,951 bytes；按出现序号连接得到 3,024/3,024 行，零漏连；
...
- 获取：官方 WebODV 提取服务；运行 `scripts/acquire_geotraces_idp2025.py --accept-fair-use`，冻结选择参数、cookies/CSRF 会话内的响应身份和全部成员；
- 当前快照：只选择深度和 dissolved Cu/Ni/Zn；69,704 个样品深度行、39,327 条非空目标观测；
- 校验：验证 903,710-byte ZIP、242 个成员、主 ODV schema、行数、变量、QC 和空间/深度覆盖；
...
- 校验：每个 range 校验 ZIP header、成员身份、展开大小、schema、行数和关键统计；完整归档登记为 201,278,791 bytes；
...
- 文件：六个官方 ZIP 均固定文件名、字节数和成员清单，解析 22 个元素分析 CSV、16,364 个物理行和 48,504 条 As/Cr/Cu/Hg/Ni/Pb/Zn 测定映射；
...
- 传输：GTK 旧站的标准 HTTPS 证书验证当前失败，实际归档端点为官方 HTTP。该适配器仅对固定 `weppi.gtk.fi` URL 启用来源级例外，拒绝重定向，并在发布缓存前核对登记文件身份、字节数、成员、schema 和行数；通用下载器仍保持 HTTPS-only。
...
- 获取：三个 `format=original` datafile 端点，分别为 847,634-byte CSV、21,024-byte variables XLSX 和 10,445-byte DL/QL XLSX；每个文件核对 file ID、文件名、字节数、schema 和行数；
...
- 获取：官方 metadata POST、文件清单 GET 和 file-ID POST 下载已验证；`Soil dataset.zip` 为 1,828,683 bytes；
- 成员：`Soil dataset.xlsx`、`Soil bulk density.xlsx`、`Description of the dataset.docx` 三项均登记文件名和字节数；完整第三方文件只进入本地缓存，不提交 Git；
...
- `source_record_id` 使用 `source_id + native_id + source_locator` 的可读、可转义组合；
- `record_id` 使用来源记录 ID、原始分析物和重复序号的可读组合；原始值不作为 ID，避免值更正时伪造新实体；
...
- 通用下载要求 HTTPS、超时、有限重试、最大字节、内容类型和响应形状检查。若发布方历史端点只有 HTTP，必须在单个适配器中限定固定主机/URL、拒绝重定向，并检查文件身份、大小、成员、schema 和行数；不得放宽通用下载器。
- 不计算或验证内容哈希。发布方公布的 checksum 可作为原始元数据存档，但不得参与本项目的路由或验收。
...
该步骤只验证并打包置信度版本、输入文件身份、字节数、记录数和报告关联，不改变 D2 的置信度算法或数值。
