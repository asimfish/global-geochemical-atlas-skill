...
> V4 政策：不再计算或校验 MD5、SHA-256 等内容哈希。下文某些来源条目中的历史 checksum/哈希只是 2026-08-05 旧审计的兼容记录，新流程不读取、比对或更新它们，也不用它们准入、评分、缓存或验收。当前追踪依据是 DOI/PID、版本、文件 ID/名称、发布/访问时间、字节数、schema、行数和关键统计。
