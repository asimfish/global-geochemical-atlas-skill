...
当前结果为 D1/D2/D3 共 106 项契约检查通过、self-test 68 项通过。D2 生成 `d2-confidence-v2` 及分量定义；`d2-interface-v2` 与 `d2-robust-mad-v2` 分别锁定异常输出接口和算法版本，并保留 `source_qualifier_raw` 与 canonical `value_qualifier`。D1 只验证来源 sidecar、acquisition manifest、输入和报告哈希，不重新计算置信度。D3 测试会从完整 D1/D2 输出目录读取任务配置，调用 `render_visualization.py`，并检查配置已应用、默认视图为任务驱动模式、标题已写入地图，以及 `visualization_report.json` 的接口版本与状态。
