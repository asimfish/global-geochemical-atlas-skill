...
`research_mode` 是叠加在一次**已完成并通过校验**的图谱运行之上的确定性后处理层。它不采集新数据、不修改任何核心产物、不改变十六文件契约，只把已有证据重组为三类面向气候与地球科学研究者的产品：
...
- `--research-dir`：研究产品输出目录，默认 `OUTPUT_DIR/research`；不得写回核心十五/十六文件所在的根目录文件名。
