...
预期为 736 条真实来源最小观测、四介质、十四来源、736/736 标准化和有效坐标、20 条已确认删失值、89 个隔离背景组及完整九文件输出。FOREGS 中可能的 `DL/2` 数值和 AfSIS 中低于来源 DL/QL 的已发布数值都只带证据边界，不冒充普通可靠检出。演示时先展示覆盖空白和来源范围，再展示地图；16 个候选异常只用于说明筛查界面，不能作污染、矿化或区域元素丰亏结论。
