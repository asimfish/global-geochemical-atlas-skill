...
4. Africa、Asia、Europe、North America、Oceania、South America 各自至少占用其陆地单元数的 10%，且不少于 2 个 canonical 单元。
5. 全部非极地陆地至少占用 15% 的 canonical 单元。
...
失败时，控制器把最后一轮 `sufficiency.discovery_gaps.spatial_dimension_gaps` 与 `geographic_search_targets` 扁平化为符合 [schema](d1-repair-queue.schema.json) 的 `d1_repair_queue.json`。筛选视图目标优先于整体目标；整体目标再按 `core → extended → macroregion_backfill → global_backfill` 排序。每项都给出 canonical/reported 事实、阈值、缺口原因、候选来源、首选修复模式、fallback chain 和完成证据要求：

| `repair_mode` | 必须执行的动作 |
|---|---|
| `rerun_current_controller` | 当前 v5 报告的充分性评估缺失或失败；用相同冻结请求在当前 Skill 下重跑评估并重建队列。旧版 `loop_report` 不得原地恢复，须换新输出目录重算。 |
...
非全球 production 请求也不能只靠一个局部高密度簇过关。引擎只看解析后的 polygon/bbox 尺度，从策略允许的 5°、2°、1°、0.5°、0.25°中选择分辨率，目标是在长轴形成约十格；命名国家与同范围 bbox 使用同一策略引擎，前者额外做精确国界点内判定。整体视图必须有至少 20 个 canonical 独立样品，并占用 `max(2, ceil(目标 coverage cells × 15%))` 个单元；国家范围的目标格落在国界内，任意 bbox 则保留范围内陆地与水域，因此海域研究不会被误判为空。各元素、介质和组合再按策略比例独立检查。失败同时写入兼容的 `regional_spatial_gap` 和通用 `spatial_dimension_gaps`。若 reported-only 已达到门槛，先核验 CRS/datum，不能证明时转向带 canonical 坐标的新来源。只有在最细允许网格下仍不足两个目标格的微小范围才标记 `unassessable`，继续由精确点内判定、bbox 裁剪和地图渲染门禁负责，绝不把“无法粗粒度评估”写成通过。
