...
实测（2 CPU 容器）端到端 <10 秒，适合快速确认接口、科学规则和渲染器没有回归。真实研究仍必须先在线获取：每轮默认 30 分钟，若 TPDC 或其他兼容来源尚未完成且充分性门禁仍有可修复缺口，则继续下一轮，累计不超过 12 小时。达到上限仍不足时保留已完成来源证据并诚实返回检查点；fixture 不得冒充在线结果。
