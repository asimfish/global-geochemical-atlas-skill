...
hash 固定的中国区域双源 fixture，服务 bbox 73–135E、18–54N 的土壤+沉积物请求，在 900 秒沙箱预算内免下载执行。运行入口：
...
实测（2 CPU 容器）端到端 <10 秒，远低于 600 秒验收线；此前同题在线路径实测 1,216 秒（TPDC 官方 ZIP 下载+哈希校验+xlsx 解析），超过 900 秒沙箱时限，这就是 fixture 优先的原因。

## 来源登记

### 1. TPDC 中国山地土壤剖面（全量 6,570 观测）
...
- 切片：1,314 个 O/A/C 层位样品 × Cr/Cu/Ni/Pb/Zn = 6,570 观测（注册 `expected_counts` 全量，无 As/Hg）；
...
### 2. Zenodo 7098563 长江/黄河沉积物（全量 558 观测）
...
`fixtures/china/combined-v1/run_manifest.json` 绑定：请求、两源逐 fixture 输入/证据 SHA-256、逐源 generation manifest 摘要、源文件哈希绑定、7,128 条 record_id 与证据 1:1 对账、介质/元素计数、比较隔离边界（O/A/C 层位、HCl/AC 残余、粒级、坐标政策）。`run_atlas_request.py --demo china` 在执行前重新核验 fixture 输出哈希（`offline_fixture_hash_verified`）。
