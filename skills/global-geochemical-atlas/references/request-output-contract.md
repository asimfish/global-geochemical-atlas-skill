...
独立 D3 产物在核心十八文件之外按 profile 条件生成：元素组合任务生成 `element_comparison.json`，单元素任务生成 `concentration_grid.geojson`。二者均由 `visualization_report.json` 路径和 SHA-256 绑定，分别遵循 [element-comparison.schema.json](element-comparison.schema.json) 与 [concentration-grid.schema.json](concentration-grid.schema.json)。
...
不删除或合并 canonical `geochemistry.csv` 记录。为了让十万级产物在普通浏览器中可用且不越过 100 MB 单文件门禁，
`samples.geojson` 与 HTML 按 96 MB 字节预算自适应内嵌：优先内嵌全部可绘测定，超预算时才由覆盖保留算法确定性收缩，并以 `record_id` 连接完整标准库。
预览必须保留异常候选和完整物理样品组，并在 `run_summary.json` 记录 canonical/可绘/已内嵌数量与策略；
所有完整性统计、声明台账、证据链和 SHA-256 绑定仍以 canonical CSV 为准，预览不得被宣称为全量数据库。浓度色阶的启用条件固定为单一元素、
