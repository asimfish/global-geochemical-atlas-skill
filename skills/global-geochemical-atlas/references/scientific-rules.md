...
| 字段 | 含义 |
|---|---|
| `record_id` | 来源内或管线内唯一测定 ID；缺失时脚本生成稳定 ID 并加 flag |
| `sample_id` | 原始样品 ID |
| `element_or_analyte` | 元素符号、元素名或原始分析物名 |
| `value` | 原始数值或带 qualifier 的原始字符串，例如 `<0.1`、`BDL` |
| `unit` | 原始单位 |
| `medium` | `rock`、`soil`、`sediment`、`water` 等介质 |
| `measurement_basis` | `dry_total`、`wet_total`、`dissolved`、`extractable`、`exchangeable` 等 |
| `source_id` | 数据集、文献或数据库记录标识 |
| `source_locator` | DOI、URL、表/页/记录号等可复查定位 |

### 推荐列

`value_qualifier`、`detection_limit`、`detection_limit_unit`、`latitude`、`longitude`、
`source_crs`、`coordinate_uncertainty_m`、`geologic_unit`、`analytical_method`、
`digestion_or_extraction`、`laboratory`、`license`、`source_tier`、`sampled_at`、
`sample_depth_min_m`、`sample_depth_max_m`、`grain_fraction`。

输出另保留 `original_latitude_raw` 与 `original_longitude_raw`。无效或疑似交换的坐标不会写入 canonical
`latitude`/`longitude`，但原始表达不会丢失。

允许保留额外列；脚本只输出 canonical 字段，D1 应在 source manifest 保存源列映射。
...
- 值与单位：`MISSING_VALUE`、`INVALID_NUMERIC_VALUE`、`INVALID_DETECTION_LIMIT`、`NEGATIVE_CONCENTRATION`、`UNSUPPORTED_UNIT`、`AMBIGUOUS_AQUEOUS_RATIO_UNIT`、`CENSORED_VALUE`、`NONDETECT_WITHOUT_LIMIT`。
...
来源分量的 v1 起始值为：官方策展 1.00、政府机构 0.95、同行评审 0.90、机构仓储 0.80、作者补充材料 0.75、聚合站 0.55、未知 0.30；缺少 source ID、精确定位或许可会继续扣分。来源 tier 必须由 D1 根据来源与版本信息显式填写，不按域名自动猜测。
...
默认分组字段：`element_or_analyte`、`medium`、`measurement_basis`、`geologic_unit`、`analytical_method`、`digestion_or_extraction`。缺少方法字段的记录会落入显式 `null` 组并降低置信度；不同已知方法默认不混合。

处理步骤：

1. 只使用成功标准化、非删失、非重复候选、正的 `normalized_value`。
2. 每个背景组至少 8 条有效记录；不足则输出 `insufficient_group_size`。
3. 对浓度取 `log10`。
4. 计算中位数和原始 MAD。
...
8. 输出背景组字段、有效 n、排除数、median、MAD、阈值和方法版本。
