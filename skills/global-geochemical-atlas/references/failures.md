...
| 状态 | 使用条件 | 调用方动作 |
|---|---|---|
| `invalid_input` | URL 非 HTTPS、目标为本机或私网、参数或版本格式无效 | 修正请求；不重试 |
...
| `incomplete_retrieval` | 版本、文件身份、大小、成员、字段、行数、格式或解压校验失败 | 停止使用该文件；核对官方来源和注册表 |
...
| 场景 | 检测与行为 | 允许的降级 |
|---|---|---|
| 网络失败，有有效缓存 | 按 URL、DOI/PID、数据集版本、文件身份、字节数、schema 和行数验证后返回 `cache_hit`，不联网 | 使用缓存，并记录 `cache_verified_at` |
...
| 空文件、文件损坏、字节数或必需成员不一致 | 不发布正式文件，不更新成功 manifest | 核对注册表和官方版本 |
| 缓存 URL、DOI/PID、版本、文件身份、字节数、schema 或行数不匹配 | 不复用缓存 | 使用明确的 `--refresh` 获取注册版本，或人工更新注册表 |
...
测试使用内存响应、临时文件和故障注入覆盖缓存缺失、版本/文件身份/schema/行数不一致、HTML、声明和流式大小超限、超时重试、403 停止、有限 HTTP 重试、ZIP 损坏与路径穿越、必要成员和字段缺失。测试不会访问受限来源，也不会在仓库内写入第三方完整数据。
