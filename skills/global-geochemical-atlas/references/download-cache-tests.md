...
> 历史测试记录：本文保留 2026-08-05 当时的测试输入和结果，不是 V4 操作指南。V4 不再计算或校验内容哈希；当前缓存验收使用 DOI/PID、版本、URL、文件 ID/名称、字节数、schema、行数和关键统计。
...
该结果证明提交内容不依赖开发工作区中的未提交文件，并作为下方 E1 在线、缓存、离线和故障验收的前置验证。

## 2026-08-05 15:41 CST

### E1 在线、缓存、离线与故障验收

在新的临时 Git 检出中清空继承环境变量，并使用新的 `/tmp` 缓存目录执行验收。第一次权限审批服务超时发生在命令启动前；按环境提示重试后进入实际数据测试，该审批超时不计作来源失败。

真实在线获取结果：

| 文件 | 字节数 | 状态 | SHA-256 |
|---|---:|---|---|
| `Appendix_2b_Top5_18Sept2013.txt` | 1,450,062 | `downloaded` | `e831095cba4549a87144804aa1ccb38e07d44550ab91b27390b84fc1dbdbc894` |
| `Appendix_3b_Ahorizon_18Sept2013.txt` | 1,987,189 | `downloaded` | `462821c1164b5b4c27905a3ea62a22fa3f6172da09125d8f80ec5b2f6a69a202` |
| `Appendix_4b_Chorizon_18Sept2013.txt` | 1,996,147 | `downloaded` | `4aa8080f81f690587f762300bada16e33e376eea0003243d5333b47c78b4c90c` |

随后使用同一目录执行 `cached` 模式，三个文件全部返回 `cache_hit`；解析器保留土层、原始字段、单位、文件名和物理行号。
