...
|---:|---|---|---|---|---|
| 1 | `geotraces-idp2025` | 首次补入固定版本、全球海洋、质量控制明确的水体候选 | 中 | 航次/深度/分相语义复杂；BODC 归档尚未准备完成 | 继续轮询官方归档；拿到后固定成员清单、hash、变量、QC flags、贡献者和原文引用，再做最小适配 |
| 2 | `norway-marchem` | 直接补海洋沉积物；公开 API、许可、目标元素和方法元数据已核实 | 中 | 当前是动态导出；部分消解、批次认可状态、参数组重复行必须保留 | 先固定 DOI 或审阅快照版本政策；再实现逐参数/批次适配器并完成人工 30 条回看 |
...
1. 先处理没有任何 `normalized_analysis` 来源的介质；
2. 再扩大当前单一区域或单一来源覆盖；
