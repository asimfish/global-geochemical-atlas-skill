# 复制给空白 Qwen3.8-Max Agent：不使用 Skill 组
...
不得读取指定仓库的参考实现、gold、workstreams、历史运行结果或 `skills/` 目录。
...
git sparse-checkout set '/evaluation/agent_uplift/**'
git checkout evaluation-stage-benchmark-v2
git rev-parse HEAD
```

完整读取 `evaluation/agent_uplift/TASK.md`，在
`evaluation/agent_uplift/experiment/no_skill/` 完成同一任务。
...
- 不得扩展 sparse checkout；不得读取或恢复 `skills`、`evaluation/stage_benchmark`、`workstreams`、gold、Git 历史参考实现。
...
python evaluation/agent_uplift/public_case/prepare_case.py \
  --output-dir evaluation/agent_uplift/experiment/no_skill/case_data
# 按 TASK.md 生成 experiment/no_skill/submission
python evaluation/agent_uplift/public_case/score_submission.py \
  --submission-dir evaluation/agent_uplift/experiment/no_skill/submission \
  --output evaluation/agent_uplift/experiment/no_skill/score.json
```
