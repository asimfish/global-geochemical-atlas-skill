...
唯一允许的 Skill 是指定仓库分支中的 `global-geochemical-atlas`。不得读取该仓库的参考实现、gold、workstreams 或历史运行结果。
...
git sparse-checkout set '/skills/global-geochemical-atlas/**' '/evaluation/agent_uplift/**'
...
任务使用的 Skill。然后完整读取 `evaluation/agent_uplift/TASK.md`，在仓库外或 `evaluation/agent_uplift/experiment/with_skill/`
完成任务。
...
- 不得扩展 sparse checkout；不得读取或恢复 `evaluation/stage_benchmark`、`workstreams`、gold、Git 历史中的参考实现。
...
```bash
python evaluation/agent_uplift/public_case/prepare_case.py \
  --output-dir evaluation/agent_uplift/experiment/with_skill/case_data
# 按 TASK.md 生成 experiment/with_skill/submission
python evaluation/agent_uplift/public_case/score_submission.py \
  --submission-dir evaluation/agent_uplift/experiment/with_skill/submission \
  --output evaluation/agent_uplift/experiment/with_skill/score.json
```
