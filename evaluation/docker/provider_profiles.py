...
def _validate_evidence(items: Any, profile_id: str) -> None:
    ...
    for index, item in enumerate(items):
        ...
        if evidence_id in seen:
            raise ProviderProfileError(
                f"profile {profile_id!r} has duplicate evidence id {evidence_id!r}"
            )
        ...
        try:
            ...
            raise ProviderProfileError(
                f"{field}.accessed_at must be YYYY-MM-DD"
            ) from exc
...
def validate_registry(value: Any) -> dict[str, Any]:
    ...
    if value.get("schema_version") != SCHEMA_VERSION:
        raise ProviderProfileError(
            f"unsupported provider profile schema: {value.get('schema_version')!r}"
        )
    ...
    for profile_id, entry in profiles.items():
        ...
        if not isinstance(entry.get("runnable"), bool):
            raise ProviderProfileError(
                f"profiles.{profile_id}.runnable must be boolean"
            )
        verification_status = _nonempty_string(
            entry.get("verification_status"),
            f"profiles.{profile_id}.verification_status",
        )
        if verification_status not in VERIFICATION_STATUSES:
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid verification status"
            )
        if not isinstance(entry.get("official_claim"), bool):
            raise ProviderProfileError(
                f"profiles.{profile_id}.official_claim must be boolean"
            )
        transport = _nonempty_string(
            entry.get("transport"), f"profiles.{profile_id}.transport"
        )
        if transport not in {"openai-compatible", "unknown"}:
            raise ProviderProfileError(
                f"profile {profile_id!r} has unsupported transport {transport!r}"
            )
        endpoint = _policy(
            entry.get("endpoint_policy"), f"profiles.{profile_id}.endpoint_policy"
        )
        model = _policy(
            entry.get("model_policy"), f"profiles.{profile_id}.model_policy"
        )
        temperature = _policy(
            entry.get("temperature_policy"), f"profiles.{profile_id}.temperature_policy"
        )
        thinking = _policy(
            entry.get("thinking_policy"), f"profiles.{profile_id}.thinking_policy"
        )
        if endpoint["kind"] not in {"absolute_https", "unknown"}:
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid endpoint policy"
            )
        _reject_unknown_fields(
            endpoint, {"kind"}, f"profiles.{profile_id}.endpoint_policy"
        )
        if model["kind"] not in {"exact", "safe_id", "unknown"}:
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid model policy"
            )
        model_fields = (
            {"kind", "value"} if model["kind"] == "exact" else {"kind", "display_name"}
        )
        _reject_unknown_fields(
            model, model_fields, f"profiles.{profile_id}.model_policy"
        )
        if model["kind"] == "exact":
            exact_model = _nonempty_string(
                model.get("value"), f"profiles.{profile_id}.model_policy.value"
            )
            if not SAFE_MODEL_ID.fullmatch(exact_model):
                raise ProviderProfileError(
                    f"profile {profile_id!r} has unsafe exact model id"
                )
        if temperature["kind"] not in {"exact", "range", "unknown"}:
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid temperature policy"
            )
        temperature_fields = {
            "exact": {"kind", "value"},
        ...
        _reject_unknown_fields(
            temperature, temperature_fields, f"profiles.{profile_id}.temperature_policy"
        )
        if temperature["kind"] == "exact" and not _is_number(temperature.get("value")):
            raise ProviderProfileError(
                f"profile {profile_id!r} exact temperature must be finite and numeric"
            )
        if temperature["kind"] == "range":
            if not all(
                _is_number(temperature.get(key)) for key in ("minimum", "maximum")
            ):
                raise ProviderProfileError(
                    f"profile {profile_id!r} temperature range must be finite and numeric"
                )
            if temperature["minimum"] > temperature["maximum"]:
                raise ProviderProfileError(
                    f"profile {profile_id!r} temperature range is reversed"
                )
        if thinking["kind"] not in {"exact", "unknown"}:
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid thinking policy"
            )
        _reject_unknown_fields(
            thinking,
            {"kind", "value", "semantics"},
            f"profiles.{profile_id}.thinking_policy",
        )
        expected_thinking = (
            "unknown" if thinking["kind"] == "unknown" else thinking.get("value")
        )
        if (
            thinking.get("value") != expected_thinking
            or thinking.get("value") not in THINKING_MODES
        ):
            raise ProviderProfileError(
                f"profile {profile_id!r} has invalid thinking value"
            )
        _nonempty_string(
            thinking.get("semantics"),
            f"profiles.{profile_id}.thinking_policy.semantics",
        )
        if entry["runnable"]:
            if transport != "openai-compatible":
                raise ProviderProfileError(
                    f"runnable profile {profile_id!r} has unsupported transport"
                )
            if endpoint["kind"] != "absolute_https":
                raise ProviderProfileError(
                    f"runnable profile {profile_id!r} must require absolute HTTPS"
                )
            if model["kind"] not in {"exact", "safe_id"}:
                raise ProviderProfileError(
                    f"runnable profile {profile_id!r} has unresolved model policy"
                )
            if temperature["kind"] not in {"exact", "range"}:
                raise ProviderProfileError(
                    f"runnable profile {profile_id!r} has unresolved temperature policy"
                )
            if thinking["kind"] != "exact" or thinking.get(
                "value"
            ) not in THINKING_MODES - {"unknown"}:
                raise ProviderProfileError(
                    f"runnable profile {profile_id!r} has unresolved thinking policy"
                )
...
class ProviderProfile:
    ...
    def validate_runtime(
        ...
        *,
        ...
        if not isinstance(temperature, (int, float)) or not math.isfinite(
            float(temperature)
        ):
        ...
        if not model_id or not SAFE_MODEL_ID.fullmatch(model_id):
            raise ProviderProfileError(
                "model id is empty or contains unsafe characters"
            )
        ...
        if temperature_policy["kind"] == "exact" and value != float(
        ...
        if temperature_policy["kind"] == "range" and not (
            float(temperature_policy["minimum"])
            <= value
            <= float(temperature_policy["maximum"])
        ):
            raise ProviderProfileError(
    ...
    def verify_frozen_hashes(
        self, *, registry_sha256: str, profile_sha256: str
    ) -> None:
        if registry_sha256 != self.registry_sha256:
            raise ProviderProfileError(
                "provider profile registry hash drift detected inside the container"
            )
        if profile_sha256 != self.profile_sha256:
            raise ProviderProfileError(
                f"provider profile {self.profile_id!r} hash drift detected inside the container"
            )
...
def load_provider_profile(
    profile_id: str, registry_path: Path = REGISTRY_PATH
) -> ProviderProfile:
    ...
    try:
        ...
        raise ProviderProfileError(
            f"cannot load provider profile registry: {registry_path}"
        ) from exc
    entry = registry["profiles"].get(profile_id)
    if entry is None:
        available = ", ".join(sorted(registry["profiles"]))
        raise ProviderProfileError(
            f"unknown provider profile {profile_id!r}; available: {available}"
        )
