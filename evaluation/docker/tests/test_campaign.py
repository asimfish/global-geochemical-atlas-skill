...
class CampaignTests(unittest.TestCase):
    ...
    def test_campaign_plan_records_audited_provider_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            plan = json.loads(
                (output / "campaign_plan.json").read_text(encoding="utf-8")
            )
            self.assertEqual(plan["provider_profile"]["id"], "local-qwen38-openai-v1")
            self.assertEqual(
                plan["provider_profile"]["thinking"]["mode"], "not_configured"
            )
            self.assertEqual(
                plan["provider_profile"]["runtime_policy"]["model"]["value"],
                "qwen3.8-max",
            )
    ...
    def test_inline_reviews_fail_before_docker_because_bindings_do_not_exist_yet(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_selection_and_candidate_status(self) -> None:
        ...
        self.assertEqual(
            candidate_status(0, False, ["artifacts/map.png"]), (74, "failed")
        )
    ...
    def test_loopback_proxy_uses_host_build_network_without_serializing_secrets(
        self,
    ) -> None:
    ...
    def test_candidate_relay_token_is_not_serialized_in_docker_arguments(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            campaign_root.mkdir()
            relay = ProviderRelayHandle(
                container="relay", token="relay-capability-token"
            )
            score = {
            ...
            with (
                ...
                patch(
                    "campaign.execute_container", return_value=(0, False, 0.1)
                ) as execute,
                ...
                run_one(
                    ...
                    agent_command=[
                        "python",
                        "/opt/evaluation/container/run_opencode.py",
                    ],
                    network=NetworkHandle(
                        mode="whitelist",
                        internal="internal",
    ...
    def test_llm_report_resolution_is_per_run(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                locate_llm_report(root, "run", "main", "Q01", "S0", 2), report
            )
    ...
    def test_llm_review_must_bind_to_the_exact_run(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path.write_text(
                json.dumps({"task_id": "GGA-SOURCE-001", "criteria": []}),
                encoding="utf-8",
            )
    ...
    def test_finalize_reviews_updates_an_existing_bound_run(self) -> None:
        ...
        rubric = (
            repository / "evaluation" / "release" / "public" / "Q01" / "rubric.json"
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            binding = {
                ...
                "objective_report_sha256": hashlib.sha256(
            ...
            first_artifact = (
                run_dir
                / "submission"
                / alignment["submission"]["required_artifacts"][0]["path"]
            )
    ...
    def test_finalize_reviews_preserves_ineligible_runs_without_requesting_reports(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                json.loads((campaign / "runs.jsonl").read_text())["score"], score
            )
    ...
    def test_log_secret_redaction(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                "before [REDACTED_EVAL_SECRET] after\n",
            )
