...
def main() -> int:
    ...
    (ROOT / "observations.csv").write_text(
        "record_id,element_or_analyte,value,unit,medium\n", encoding="utf-8"
    )
    ...
    (ROOT / "anomalies.geojson").write_text(
        json.dumps(empty_geojson, sort_keys=True) + "\n", encoding="utf-8"
    )
    (ROOT / "map.png").write_bytes(PNG)
    (ROOT / "map.html").write_text(
        "<!doctype html><meta charset=utf-8><title>Docker smoke</title>\n",
        encoding="utf-8",
    )
    ...
    print(
        json.dumps(
            {"status": "success", "skill_visible": skill.is_file()}, sort_keys=True
        )
    )
    return 0
