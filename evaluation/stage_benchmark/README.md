...
这个目录位于 `workstreams/d2/` 之外，只作为团队内部验证工程，不是第二个参赛 Skill。
...
```bash
python3 evaluation/d2-validation/scripts/run_isolated_suite.py \
  --report /tmp/d2-isolated-report.json
```

```bash
python3 evaluation/d2-validation/scripts/run_e2e_suite.py \
...
```bash
python3 evaluation/d2-validation/scripts/run_stage_benchmark.py \
  --stage all \
...
```bash
python3 evaluation/d2-validation/scripts/run_all.py \
