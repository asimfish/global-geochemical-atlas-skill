...
| 测试域 | 主要案例 | 核心判据 |
|---|---|---|
| 接口与版本 | 版本号、八项输出、`--help`、退出码、stdout | 输入错误为 2；成功只输出路径 JSON |
...
| 确定性与 manifest | 同输入运行两次、输出哈希 | 八个文件逐字节一致；manifest 哈希匹配 |
...
D1 stub 不访问网络，只包装 `workstreams/d2/fixtures/demo_input.csv`：
...
阻断通过标准：8 个文件哈希正确、记录不丢失且 ID 唯一、所有金标准正确、删失值不插补、证据链
闭合、至少一个真实背景组完成异常筛查、D3 能离线消费全部八项输出，并且比赛要求的四项产品
...
```bash
python3 evaluation/d2-validation/scripts/run_real_data_suite.py \
...
候选脚本必须支持 `--input <csv> --output-dir <new-dir>` 并生成 `d2-interface-v2` 的八个文件。
