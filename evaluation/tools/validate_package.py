...
def task_locations(
    root: Path, private_root: Path | None = None, public_only: bool = False
) -> list[tuple[str, Path]]:
        ...
        if selected.is_dir():
            locations.extend(
                (
                    ...
                    ("final_holdout", selected / "final_holdout"),
                )
            )
    for split, base in locations:
        if base.exists():
            result.extend(
                (split, path)
                for path in sorted(base.glob("Q[0-9][0-9]"))
                if path.is_dir()
            )
    return result
...
def main() -> int:
    ...
    alignment = json.loads(
        (root / "contracts" / "benchmark-execution-contract.json").read_text(
            encoding="utf-8"
        )
    )
    required_outputs = [
    ...
    if not args.public_only and selected_private.is_dir():
        expected.update(
            {
                "shadow": ALL_EXPECTED["shadow"],
                "final_holdout": ALL_EXPECTED["final_holdout"],
            }
        )
    ...
    for split, task_dir in task_locations(root, private_root, args.public_only):
        ...
        if metadata.get("split") != split:
            errors.append(
                f"{task_dir.name}: split mismatch: {metadata.get('split')} != {split}"
            )
        ...
        if metadata.get("required_outputs") != required_outputs:
            errors.append(
                f"{task_dir.name}: required_outputs do not exactly match the E1 artifact contract"
            )
        ...
        try:
            manifest = json.loads(
                (task_dir / "gold" / "artifacts" / "run_manifest.json").read_text(
                    encoding="utf-8"
                )
            )
            ...
            missing_evidence = [
                item
                for item in metadata.get("legacy_logical_outputs", [])
                if item not in evidence
            ]
            if missing_evidence:
                errors.append(
                    f"{task_dir.name}: run_manifest benchmark_evidence missing {missing_evidence}"
                )
        ...
        if re.search(
            r"\b(TODO|TBD|PLACEHOLDER)\b|待补|后续任务", task_text, flags=re.IGNORECASE
        ):
        ...
        if rubric_path.is_file():
            try:
                ...
                criterion_points = sum(
                    int(item["points"]) for item in rubric.get("criteria", [])
                )
                ...
                allowed_dimensions = {
                    item["dimension_id"] for item in alignment["dimensions"]
                }
                bad_dimensions = [
                    item.get("id")
                ...
                if bad_dimensions:
                    errors.append(
                        f"{task_dir.name}: rubric criteria missing valid E1 dimensions: {bad_dimensions}"
                    )
            except (
                OSError,
        ...
        if grader_spec_path.is_file():
            try:
                ...
                errors.extend(
                    validate_candidate_visible_contract(task_dir, metadata, grader_spec)
                )
                candidate_contracts_checked += 1
            except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
                errors.append(
                    f"{task_dir.name}: invalid candidate-visible contract: {exc}"
                )
        ...
        try:
            ...
            if not passed:
                ...
                errors.append(
                    f"{task_dir.name}: gold smoke failed; checks={failed_ids}; redlines={report['redline_events']}"
                )
    ...
    if not inventory_paths[0].is_file():
        ...
        if len(inventory_rows) != len(task_ids):
            errors.append(
                f"inventory rows={len(inventory_rows)}, expected={len(task_ids)}"
            )
