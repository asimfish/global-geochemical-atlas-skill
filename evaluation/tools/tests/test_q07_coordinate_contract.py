...
class Q07CoordinateContractTests(unittest.TestCase):
    def test_p02_swap_rule_is_consistent_across_visible_task_checker_and_gold(
        self,
    ) -> None:
        ...
        manifest = json.loads(
            (RELEASE_Q07 / "gold" / "artifacts" / "run_manifest.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        p02_check = next(
            check for check in checker["checks"] if check["id"] == "p02_lat"
        )
        evidence_rows = {
            row["record_id"]: row
