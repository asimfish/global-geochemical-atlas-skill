...
冻结契约覆盖当前全部 14 个可执行来源，并按岩石、土壤、沉积物、水体分层独立保存：
...
离线门禁逐行检查 `sources.jsonl` 与 `demo_input.csv` 的来源身份、版本、许可证和文件哈希，并对冻结原始记录做精确比较；它不会把“哈希相同”误写成“科学事实正确”。在线模式额外解析权威入口，网络失败输出 `needs_human_review`，不会伪装成已核验。FOREGS 六介质使用参与该项目的 BGS 官方项目页作可达权威入口；GTK 原始下载与许可 URL 仍保留在来源目录，且绝不通过关闭 TLS 校验来掩盖旧站证书问题。
