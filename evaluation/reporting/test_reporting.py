...
class ReportingTests(unittest.TestCase):
    def test_q01_adapter_uses_same_report_and_marks_partial_scores_nonformal(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for number in range(1, 25):
                ...
                (task / "objective_report.json").write_text(
                    json.dumps(
                        {
                            "task_id": f"TASK-{question}",
                ...
                (task / "llm_grader_report.json").write_text(
                    json.dumps(
                        {
            ...
            self.assertEqual(
                report["schema_version"], "global-geochemical-evaluation-report-v1"
            )
            self.assertEqual(report["benchmark"]["task_report_count"], 24)
            self.assertEqual(
                report["benchmark"]["stage_summaries"]["D1"]["question_count"], 6
            )
    ...
    def test_controller_manifest_binds_pair_and_only_allows_skill_visibility_difference(
        self,
    ) -> None:
        config = json.loads(
            (
                Path(__file__).resolve().parents[2]
                / "evaluation_lyf/agent_uplift/experiment_config.json"
        ...
        b0 = {
            ...
            "files": common
            + [{"path": "AGENT_PROMPT.md", "bytes": 1, "sha256": "c" * 64}],
        }
        s0 = {
            **b0,
            "condition": "S0",
            ...
            + [
                ...
                {
                    "path": ".agents/skills/global-geochemical-atlas/SKILL.md",
                    "bytes": 2,
                    "sha256": "f" * 64,
                },
            ],
        ...
        self.assertEqual(
            b0_manifest["pair_fingerprint"], s0_manifest["pair_fingerprint"]
        )
        self.assertNotEqual(
            b0_manifest["run_fingerprint"], s0_manifest["run_fingerprint"]
        )
        release_commit = "9" * 40
        ...
        self.assertEqual(
            release_commit, release_manifest["common_experiment"]["commit"]
        )
        s0["files"][0] = {"path": "TASK.md", "bytes": 11, "sha256": "0" * 64}
        with self.assertRaisesRegex(
            build_experiment_manifest.ManifestError, "public task bytes differ"
        ):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            rows = [
                {
                    ...
                    "latitude": "-31.93",
                    "longitude": "115.85",
                    "original_latitude_raw": "-31.93",
                    "original_longitude_raw": "115.85",
                    "normalized_value": "1",
                    "normalized_unit": "mg/kg",
                    "source_id": "source-1",
                    "source_locator": f"https://example.test#row={i}",
                    "file_sha256": "0" * 64,
                    "license": "CC0",
                    "qc_flags": "[]",
                    "operational_confidence": '{"band":"high"}',
                    "analytical_method": "ICP-MS",
                    "geologic_context_status": "reported",
            ...
            samples = {
                ...
                "features": [
                    {
                        "type": "Feature",
                        "geometry": {"type": "Point", "coordinates": [115.85, -31.93]},
                        "properties": {"record_id": row["record_id"]},
                    }
                    for row in rows
                ],
            }
                ...
                reports.append(
                    {
                        ...
                        "experiment": {
                            "condition": condition,
                            "run_id": f"{condition}-{index}",
                            "manifest": {"pair_fingerprint": "pair-1"},
                        },
    ...
    def test_aggregate_keeps_partial_two_arm_comparison_explicitly_diagnostic(
        self,
    ) -> None:
        reports = []
            ...
            reports.append(
                {
                    ...
                    "benchmark": {
                        "task_results": [
                            {
                                "question_id": "Q01",
                                "descriptive_total_score": task_score,
                            }
                        ],
                        "stage_summaries": {
                            "D1": {"descriptive_partial_mean": task_score}
                        },
                    },
        ...
        self.assertEqual(
            diagnostic["task_win_tie_loss"], {"S0_win": 1, "tie": 0, "S0_loss": 0}
        )
