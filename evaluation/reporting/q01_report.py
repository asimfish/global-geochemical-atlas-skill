...
STAGES = {
    ...
    "D2": {"Q03", "Q04", "Q06", "Q08", "Q09", "Q12", "Q13", "Q14", "Q18", "Q19", "Q21", "Q23"},
...
def _stage(question: str) -> str:
    return next((stage for stage, questions in STAGES.items() if question in questions), "unknown")
...
def _task_results(results_root: Path) -> list[dict[str, Any]]:
    ...
    for number in range(1, 25):
        ...
        present = all((task_root / name).is_file() for name in ("score.json", "objective_report.json", "llm_grader_report.json"))
        results.append(
            {
...
def _stage_summaries(task_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for stage in STAGES:
        ...
        partial = [float(item["descriptive_total_score"]) for item in members if isinstance(item["descriptive_total_score"], (int, float))]
        summaries[stage] = {
            "question_count": len(members),
            "machine_awarded": machine_awarded,
            "machine_possible": machine_possible,
            "machine_rate": round(machine_awarded / machine_possible, 6) if machine_possible else None,
            "llm_awarded": llm_awarded,
            "llm_possible": llm_possible,
            "llm_rate": round(llm_awarded / llm_possible, 6) if llm_possible else None,
            "descriptive_partial_mean": round(sum(partial) / len(partial), 6) if partial else None,
...
def _geojson_metrics(value: Any) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or value.get("type") != "FeatureCollection" or not isinstance(features, list):
    ...
    return {"valid": valid, "features": len(features), "unique_locations": len(locations)}
...
def _screenshots_verified(audit: dict[str, Any], audit_path: Path | None) -> bool:
    ...
    if not isinstance(directory, str) or Path(directory).name != directory or not isinstance(screenshots, list) or not screenshots:
        return False
...
def build(args: Any) -> dict[str, Any]:
    ...
    all_hard_gates = all(item["hard_gate_passed"] for item in task_results if item["present"]) and score_files == 24
    partial_scores = [float(item["descriptive_total_score"]) for item in task_results if isinstance(item["descriptive_total_score"], (int, float))]
    ...
    for row in observations:
        ...
        if -90 <= lat <= 90 and -180 <= lon <= 180:
            locations.add((round(lat, 8), round(lon, 8)))
    normalized = sum(
        row.get("normalized_value") not in (None, "") and bool(row.get("normalized_unit"))
        for row in observations
        if isinstance(row, dict)
    )
    censored = sum(str(row.get("censored", "")).casefold() in {"true", "1", "yes"} for row in observations if isinstance(row, dict))
    provenance = (
        sum(bool(row.get("record_id") and row.get("source_id")) for row in observations if isinstance(row, dict)) / len(observations)
        if observations else 0.0
    )
    map_metrics = _geojson_metrics(map_geojson)
    anomaly_metrics = _geojson_metrics(anomaly_geojson)
    directions = Counter()
    for feature in anomaly_geojson.get("features", []) if isinstance(anomaly_geojson, dict) else []:
        label = str(feature.get("properties", {}).get("anomaly_type", "unknown")).casefold()
        directions["high" if "enrich" in label or "high" in label else "low" if "deplet" in label or "low" in label else "unknown"] += 1
    ...
    browser_bound = (
        ...
        and browser.get("html", {}).get("bytes") == (map_html.stat().st_size if map_html.is_file() else 0)
    ...
    confidence_components = confidence.get("components") if isinstance(confidence, dict) else None
    confidence_valid = (
        isinstance(confidence_components, dict)
        and {"source_integrity", "normalization", "spatial", "quality_control", "provenance"} <= set(confidence_components)
        and "not a probability" in str(confidence.get("interpretation", "")).casefold()
    )
    source_valid = len(sources) == 2 and all(isinstance(item, dict) and item.get("source_id") and item.get("integrity_status") for item in sources)
    ...
    reusable_skill = bool(skill_present and "name:" in skill_path.read_text(encoding="utf-8")[:1000])
    four_runtime_usable = database_valid and source_valid and confidence_valid and anomaly_valid and browser_pass
    experiment_manifest = load_json(args.experiment_manifest, {})
    pair_fingerprint = experiment_manifest.get("pair_fingerprint") if isinstance(experiment_manifest, dict) else None
    eligible = bool(
    ...
    observed = round(sum(partial_scores) / len(partial_scores), 6) if complete_scores == 24 and len(partial_scores) == 24 else None

    red_lines: list[dict[str, Any]] = []
    if complete_scores != 24:
        red_lines.append({"code": "Q01_SCORE_DIMENSIONS_INCOMPLETE", "count": 24 - complete_scores, "severity": "error"})
    if not browser_pass:
        red_lines.append({"code": "Q24_MAP_NOT_BROWSER_VERIFIED", "severity": "error"})
    if not confidence_valid:
        red_lines.append({"code": "Q24_CONFIDENCE_EVIDENCE_MISSING", "severity": "error"})
    missing_media = sorted(FOUR_MEDIA - set(media))
    if missing_media:
        red_lines.append({"code": "Q24_IS_MICRO_FIXTURE_NOT_GLOBAL_COVERAGE", "missing": missing_media, "severity": "warning"})
    ...
    return {
        ...
        "experiment": {
            ...
            "submission_hashes": {
                ...
                for name in ("observations.csv", "geochemical.sqlite", "map.html", "anomalies.geojson", "run_manifest.json")
            },
        ...
        "fairness": {
            ...
            "ineligibility_reasons": [
                reason for condition, reason in (
                    ...
                    (not args.blind_bundle, "candidate visibility boundary not attested"),
                    ...
                    (not four_runtime_usable, "Q24 final runtime deliverables are not all browser/science usable"),
                ) if condition
            ],
        ...
        "d1": {
            ...
            "sources": sorted(str(item.get("source_id")) for item in sources if isinstance(item, dict) and item.get("source_id")),
            ...
            "coverage_report_artifact": artifact(q24 / "coverage_report.json", valid=False, usable=False, detail={"reason": "not required by Q24"}),
        },
        "d2": {
            "input_records": int(qc.get("raw_input_rows", len(observations))),
            "output_records": len(observations),
            "row_preservation": int(qc.get("raw_input_rows", len(observations))) == len(observations) + int(qc.get("excluded_integrity_rows", 0)),
            "normalized_records": normalized,
            "normalization_rate": round(normalized / len(observations), 6) if observations else 0.0,
            ...
            "confidence_interpretation": confidence.get("interpretation", "not_reported"),
        ...
        "deliverables": {
            "interactive_map": artifact(map_html, valid=map_html.is_file() and "<html" in map_html.read_text(encoding="utf-8", errors="ignore").casefold(), usable=browser_pass, detail={"browser_status": browser.get("status", "not_run"), "surface": "Q24 micro-fixture"}),
            "standardized_database": artifact(q24 / "geochemical.sqlite", valid=database_valid, usable=database_valid and normalized > 0, detail={"records": len(observations), "unique_locations": len(locations)}),
            "sources_and_confidence": artifact(q24 / "sources.jsonl", valid=source_valid and confidence_valid, usable=source_valid and confidence_valid, detail={"source_rows": len(sources), "confidence_present": bool(confidence), "confidence_valid": confidence_valid}),
            "anomaly_results": artifact(q24 / "anomalies.geojson", valid=anomaly_valid, usable=anomaly_valid, detail={"features": anomaly_metrics["features"], "directions": dict(directions)}),
            "reusable_skill_document": artifact(skill_path or Path("SKILL.md"), valid=reusable_skill, usable=reusable_skill and skill_path.stat().st_size > 3000 if skill_present else False, detail={"condition": args.condition}),
        },
        "score": {
            "observed": observed,
            "maximum": 100,
            "ceiling_saturated": args.condition == "B0" and isinstance(observed, (int, float)) and observed >= 90,
            "source": str(results_root),
            "descriptive_partial_mean": round(sum(partial_scores) / len(partial_scores), 6) if partial_scores else None,
